...
`windows-cj/windows_bindgen` is the executable Cangjie binding generator.
...
The checked-in importable generated surface is currently `windows_common`,
...
1. Keep the generator role under the `windows_bindgen` directory/package.
...
```pwsh
$env:cjHeapSize = "32GB"
cjpm run -m windows_bindgen -- --emit-winmd-json .\winmd -d <some-temp-dir>
```
...
`windows_common`. Inputs are raw `.winmd` only; the gate reads the bundled
...
- `scripts/check_workspace_setup.py` passes the active workspace setup and
  generated-output invariant checks for `windows_common`.
