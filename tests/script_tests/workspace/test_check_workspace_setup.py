...
class ActiveWorkspaceLayoutTests(unittest.TestCase):
    def test_setup_tracks_bindgen_generator_without_active_sys_package(self) -> None:
        ...
        self.assertIn("windows_metadata", setup.ACTIVE_WORKSPACE_MEMBERS)
        self.assertIn("windows_time", setup.ACTIVE_WORKSPACE_MEMBERS)
        self.assertIn("samples/services/simple", setup.ACTIVE_WORKSPACE_MEMBERS)
        self.assertNotIn(removed_sys_member, setup.ACTIVE_WORKSPACE_MEMBERS)
        self.assertNotIn("windows_targets", setup.ACTIVE_WORKSPACE_MEMBERS)
        self.assertNotIn("windows_winui3", setup.ACTIVE_WORKSPACE_MEMBERS)
        self.assertIn("windows_bindgen", setup.PACKAGE_DEPENDENCIES)
        self.assertIn("windows_metadata", setup.PACKAGE_DEPENDENCIES)
        self.assertIn("windows_time", setup.PACKAGE_DEPENDENCIES)
        self.assertNotIn(removed_sys_module, setup.PACKAGE_DEPENDENCIES)
        self.assertNotIn("windows_winui3", setup.PACKAGE_DEPENDENCIES)
    ...
    def test_service_simple_package_name_is_unique(self) -> None:
        workspace = Path(__file__).resolve().parents[3]
        service_config = setup.load_toml(workspace / "samples" / "services" / "simple" / "cjpm.toml")
        windows_config = setup.load_toml(workspace / "samples" / "windows" / "simple" / "cjpm.toml")

        self.assertEqual(service_config["package"]["name"], "windows_service_sample_simple")
        self.assertNotEqual(service_config["package"]["name"], windows_config["package"]["name"])
...
class WorkspaceSetupParserTests(unittest.TestCase):
    def test_finalizer_scan_ignores_block_comment_braces(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            source = workspace / "windows_core" / "src" / "blocked_finalizer.cj"
...
class AbiOwnershipAuditTests(unittest.TestCase):
    ...
    def test_query_interface_raw_local_owned_wrapper_is_consumed(self) -> None:
        body = (
            "\n"
            "    let node = TreeViewNode.fromAbiTake(raw)\n"
            "    try {\n"
            "        use(node)\n"
            "    } finally {\n"
            "        node.close()\n"
    ...
    def test_query_interface_raw_local_take_ownership_constructor_is_consumed(self) -> None:
        body = (
            "\n"
            "    let sc2 = IDXGISwapChain2(raw, takeOwnership: true)\n"
            "    try {\n"
            "        use(sc2)\n"
            "    } finally {\n"
            "        sc2.close()\n"
    ...
    def test_query_interface_raw_forwarded_to_out_pointer_is_consumed(self) -> None:
        body = (
            "\n"
    ...
    def test_query_interface_raw_multiline_match_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            (workspace / "cjpm.toml").write_text(
                "[workspace]\nmembers = [\"pkg\"]\n",
    ...
    def test_query_interface_raw_inline_unsafe_match_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            (workspace / "cjpm.toml").write_text(
                "[workspace]\nmembers = [\"pkg\"]\n",
    ...
    def test_query_interface_raw_stored_before_match_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            (workspace / "cjpm.toml").write_text(
                "[workspace]\nmembers = [\"pkg\"]\n",
    ...
    def test_query_interface_raw_typed_stored_before_match_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            (workspace / "cjpm.toml").write_text(
                "[workspace]\nmembers = [\"pkg\"]\n",
    ...
    def test_query_interface_raw_qualified_stored_before_match_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            (workspace / "cjpm.toml").write_text(
                "[workspace]\nmembers = [\"pkg\"]\n",
    ...
    def test_direct_query_interface_function_allowlist_is_allowed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            (workspace / "cjpm.toml").write_text(
                "[workspace]\nmembers = [\"windows_reactor\"]\n",
                encoding="utf-8",
            )
            source = workspace / "windows_reactor" / "src" / "winui" / "host.cj"
            source.parent.mkdir(parents=True, exist_ok=True)
            source.write_text(
                (
                    "package windows_reactor.winui\n"
                    "func queryApplicationInner(): Int32 {\n"
                    "    let hr = unsafe { vtblPtr.read().QueryInterface(innerRaw, iid, result) }\n"
                    "    return hr\n"
                    "}\n"
                    "func unrelated(): Unit {\n"
                    "    println(\".QueryInterface( in a string is ignored\")\n"
                    "}\n"
                ),
                encoding="utf-8",
            )

            abi.check_direct_query_interface_call_sites(workspace)

    def test_direct_query_interface_outside_allowlist_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            (workspace / "cjpm.toml").write_text(
                "[workspace]\nmembers = [\"pkg\"]\n",
            ...
            source.write_text(
                (
                    "package pkg\n"
                    "func queryManually(): Int32 {\n"
                    "    let hr = unsafe { vtblPtr.read().QueryInterface(raw, iid, result) }\n"
                    "    return hr\n"
                    "}\n"
                ),
                encoding="utf-8",
            ...
            self.assertEqual(raised.exception.code, 1)
            self.assertIn("direct QueryInterface calls must use", stderr.getvalue())
...
class ShellScriptGateTests(unittest.TestCase):
    def test_has_no_legacy_script_migration_allowlist(self) -> None:
        self.assertEqual(setup.LEGACY_SCRIPT_MIGRATION_ALLOWLIST, set())
    ...
    def test_rejects_power_shell_batch_and_cmd_scripts(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write_file(workspace, "sample/dist/ignored.cmd")
            ...
            self.assertNotIn("ignored.cmd", message)
