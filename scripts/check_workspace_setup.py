...
def check_generated_common_package(workspace: Path) -> None:
    ...
    if manifest.get("generated_by") != GENERATED_MANIFEST_HEADER:
        fail("windows_common codegen manifest must identify windows_bindgen as its source")
    manifest_file_list = manifest.get("files") or []
    if len(manifest_file_list) != len(set(manifest_file_list)):
        fail("windows_common codegen manifest must not contain duplicate file entries")
    manifest_files = set(manifest_file_list)
    if not manifest_files:
        fail("windows_common codegen manifest must list generated files")
    manifest_hashes = manifest.get("file_hashes")
    if not isinstance(manifest_hashes, dict):
        fail("windows_common codegen manifest must include per-file content hashes")
    ...
    if manifest_hash_files != manifest_files:
        ...
        fail(f"windows_common codegen manifest hash key mismatch; missing={missing}, extra={extra}")
    ...
    if actual_files != manifest_files:
        ...
        fail(f"windows_common generated file set mismatch; missing={missing}, extra={extra}")
    ...
    for relative in sorted(manifest_files):
        ...
        if expected_hash != actual_hash:
            fail(f"{relative} content hash does not match windows_common codegen manifest")
        if file.suffix == ".cj":
...
def check_required_set_last_error_bool_wrappers(
    ...
    manifest_path = workspace / "windows_common" / "codegen-manifest.json"
...
def check_required_set_last_error_bool_false_success_wrappers(
    ...
    if missing:
        fail(f"windows_common missing SetLastError Bool false-success checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError Bool false-success checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError Bool status checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError Bool status checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError null-handle ABI checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError null-handle ABI checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError invalid-handle ABI checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError invalid-handle ABI checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError null-success handle ABI checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError null-success handle ABI checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError WAIT_EVENT checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError WAIT_EVENT checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError null-pointer checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError null-pointer checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError TLS value checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError TLS value checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError UInt32 -1 checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError UInt32 -1 checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError UInt32 zero checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError UInt32 zero checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError UInt32 zero-last-error checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError UInt32 zero-last-error checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError UIntNative zero checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError UIntNative zero checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError IntNative zero-last-error checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError IntNative zero-last-error checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError Int32 max checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError Int32 max checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing SetLastError Int32 -1 checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid SetLastError Int32 -1 checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing WinSock INVALID_SOCKET checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid WinSock INVALID_SOCKET checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing WinSock BOOL checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid WinSock BOOL checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing WinSock WAIT_EVENT checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid WinSock WAIT_EVENT checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing WinSock WSA_INVALID_EVENT checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid WinSock WSA_INVALID_EVENT checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing WinSock null-handle checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid WinSock null-handle checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing WinSock null-pointer checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid WinSock null-pointer checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing WinSock SOCKET_ERROR Int32 checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid WinSock SOCKET_ERROR Int32 checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing MSWSOCK GetLastError SOCKET_ERROR Int32 checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid MSWSOCK GetLastError SOCKET_ERROR Int32 checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing WinSock direct-error Int32 checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid WinSock direct-error Int32 checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing WinSock pending-status Int32 checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid WinSock pending-status Int32 checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing WinSock lpErrno Int32 checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid WinSock lpErrno Int32 checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing WAIT_FAILED UInt32 checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid WAIT_FAILED UInt32 checked wrapper(s): {invalid[:20]}")
            ...
            for method in type_def.get("Methods", []):
                ...
                if method_needs_direct_win32_error_code_int32_wrapper(method):
                    ...
                    for issue in issues:
                        invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing direct Win32 error-code Int32 checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid direct Win32 error-code Int32 checked wrapper(s): {invalid[:20]}")
            ...
            for method in type_def.get("Methods", []):
                ...
                if method_needs_direct_win32_error_code_uint32_wrapper(method):
                    ...
                    for issue in issues:
                        invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing direct Win32 error-code UInt32 checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid direct Win32 error-code UInt32 checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing UInt32 <= threshold checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid UInt32 <= threshold checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing HRESULT COM out ABI checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid HRESULT COM out ABI checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing HRESULT owned string out ABI checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid HRESULT owned string out ABI checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing HRESULT owned pointer out ABI checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid HRESULT owned pointer out ABI checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing HRESULT borrowed optional out ABI checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid HRESULT borrowed optional out ABI checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing HRESULT optional scalar out ABI checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid HRESULT optional scalar out ABI checked wrapper(s): {invalid[:20]}")
        ...
        for type_def in payload.get("types", []):
            ...
            for method in type_def.get("Methods", []):
                ...
                for issue in issues:
                    invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"windows_common missing multiple-success HRESULT checked wrapper(s): {missing[:20]}")
    if invalid:
        fail(f"windows_common invalid multiple-success HRESULT checked wrapper(s): {invalid[:20]}")
