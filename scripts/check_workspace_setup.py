...
GENERATED_MANIFEST_HEADER = windows_common_manifest.GENERATED_MANIFEST_HEADER
GENERATED_COMMON_IGNORED_FILES = windows_common_manifest.IGNORED_GENERATED_FILES
...
def check_generated_interface_dispatcher_failed_hresult_out_cleanup(workspace: Path) -> None:
    impl_root = workspace / "windows_common" / "src" / "impl"
    check_path_exists(impl_root, "windows_common generated impl directory")
    sources = sorted(impl_root.glob("symbols_*.cj"))
    if not sources:
        fail("windows_common generated impl directory must contain symbols_*.cj")
...
def check_generated_collection_vtable_abi_parity(workspace: Path) -> None:
    impl_root = workspace / "windows_common" / "src" / "impl"
    check_path_exists(impl_root, "windows_common generated impl sources")
    ...
    for label, fragment in forbidden_fragments.items():
        if fragment in impl_text:
            fail(f"windows_common generated collection vtable ABI still uses {label}")
    for label, fragment in required_fragments.items():
        if fragment not in impl_text:
            fail(f"windows_common generated collection vtable ABI missing {label}")
...
def check_generated_raw_event_add_wrappers_clear_tokens(workspace: Path) -> None:
    ...
    sources = sorted(impl_root.glob("symbols_*.cj"))
    if not sources:
        fail("windows_common generated impl symbols files are missing")
    ...
    if checked < 2:
        fail("windows_common generated raw event add wrapper audit saw too few wrappers")
...
def check_vtable_none_branches_clear_out_slots(workspace: Path) -> None:
    ...
    common_impl = workspace / "windows_common" / "src" / "impl"
    ...
    check_path_exists(common_impl, "windows_common generated impl directory")
    ...
    require_text_fragments(
        ...
        "windows_common generated vtable branches must clear representative out slots and translate thrown exceptions",
    )
    if (
        "try {\n                            impl.First(result)" not in common_text
        and "try {\n                            let resultHr__ = impl.First(result)" not in common_text
    ):
        fail("windows_common generated vtable branches must dispatch representative First method inside try")
    for label, start, clear_fragments in (
        ...
        pointer_branch = section_between(
            ...
            start,
            "} else {",
            f"windows_common generated {label} E_POINTER branch",
        )
        for clear_fragment in clear_fragments:
            require_text_before(
                ...
                clear_fragment,
                "windows_core.E_POINTER.value",
                f"windows_common generated {label} E_POINTER branch must clear non-null out slots before E_POINTER",
            )
    for call in (
        "impl.First(result)",
        "impl.GetMany(itemsSize, items, result)",
        ...
        if call_index < 0:
            fail(f"windows_common generated vtable branches must contain representative call {call}")
        ...
        if catch_index < 0:
            fail(f"windows_common generated vtable branch for {call} must translate WindowsException")
...
def check_generated_common_package(workspace: Path) -> None:
    generated = workspace / "windows_common"
    check_path_exists(generated / "cjpm.toml", "windows_common cjpm.toml")
    check_path_exists(generated / "codegen-manifest.json", "windows_common codegen-manifest.json")
    check_path_exists(generated / "src" / "mod.cj", "windows_common mod.cj")
    try:
        manifest = windows_common_manifest.validate_manifest_hashes(
            generated,
            expected_header=GENERATED_MANIFEST_HEADER,
        )
    except windows_common_manifest.ManifestError as error:
        fail(str(error))
    manifest_files = set(windows_common_manifest.manifest_files(manifest))
    ...
    if package.get("name") != "windows_common":
        fail(f"windows_common package name must be windows_common, got {package.get('name')!r}")
    if manifest.get("package_name") != "windows_common":
        fail(f"windows_common manifest package name must be windows_common, got {manifest.get('package_name')!r}")
    ...
    if declared_deps != expected_support_deps:
        ...
        fail(f"windows_common cjpm.toml dependencies mismatch; missing={missing}, extra={extra}")
    if "link-option" in (generated / "cjpm.toml").read_text(encoding="utf-8"):
        fail("windows_common must not emit static link options; native calls go through windows_libloading")
...
def check_generated_common_impl_invariants(generated: Path) -> None:
    ...
    check_path_exists(impl_dir, "windows_common impl symbol directory")
    impl_files = sorted(impl_dir.glob("symbols_*.cj"))
    if not impl_files:
        fail("windows_common impl symbol directory must contain symbols_*.cj chunks")
    impl_text = "\n".join(path.read_text(encoding="utf-8") for path in impl_files)
    if "func Start(callback: CPointer<Unit>): Int32" not in impl_text:
        fail("windows_common IApplicationStatics.Start must accept a raw WinRT delegate COM pointer")
    if "public type Microsoft_UI_Xaml_ApplicationInitializationCallback = CFunc" in impl_text:
        fail("windows_common must not render WinRT delegate projections as CFunc aliases")
    for _, header, body in iter_cj_class_blocks(impl_text):
        ...
        if "<: Resource" in header or "& Resource" in header:
            ...
            for fragment in required_fragments:
                if fragment not in body:
                    fail(
                        "windows_common raw handle Resource wrappers must preserve "
        ...
        match = re.search(r"\bclass\s+([A-Za-z_][A-Za-z0-9_]*)", header)
        class_name = match.group(1) if match is not None else header.strip()
        fail(f"windows_common raw handle classes must be owning Resource wrappers: {class_name}")
    ...
    if "public open class Microsoft_UI_Xaml_ApplicationInitializationCallback <: windows_interface.InterfaceWrapperBase & windows_interface.ComInterface" not in impl_text:
        fail("windows_common WinRT delegate ApplicationInitializationCallback must project as a ComInterface delegate wrapper")
    if "private class OwnedMicrosoft_UI_Xaml_ApplicationInitializationCallback <: Microsoft_UI_Xaml_ApplicationInitializationCallback" not in impl_text:
        fail("windows_common WinRT delegate ApplicationInitializationCallback must provide an owned-handle variant for ABI cleanup")
    if "winrtBorrowGenericIn<Microsoft_UI_Xaml_ApplicationInitializationCallback>(callback)" not in impl_text:
        fail("windows_common Application.Start must borrow the delegate handle through winrtBorrowGenericIn")
    if "Microsoft_UI_Xaml_IApplication.signature()" in impl_text:
        fail("windows_common runtime class signatures must not call ambiguous interface signature() members")
    if "let _ = hr__.ok()" in impl_text:
        fail("windows_common runtime class factory wrappers must throw on failed HRESULT values")


def check_windows_common_native_checked_wrappers(workspace: Path) -> None:
    native_root = workspace / "windows_common" / "src"
    check_path_exists(native_root, "windows_common generated native helper root")
    checked_count = 0
    ...
    if inspectable_checked_count == 0:
        fail("windows_common generated native helpers must include checked Win32 wrappers")
...
def requested_win32_metadata(workspace: Path) -> tuple[Path | None, list[str]]:
    ...
    manifest_root = workspace / "windows_common"
    if not metadata_root.exists() or not (manifest_root / "codegen-manifest.json").exists():
        return None, []
    try:
        manifest = windows_common_manifest.load_manifest(
            manifest_root,
            expected_header=GENERATED_MANIFEST_HEADER,
        )
    except windows_common_manifest.ManifestError as error:
        fail(str(error))
    requested_features = windows_common_manifest.requested_features(manifest, prefix="Windows.Win32.")
    ...
    for namespace, type_def in iter_requested_win32_type_defs(metadata_payloads):
        ...
        for method in type_def.get("Methods", []):
            ...
            if name not in checked_funcs:
                missing.append(f"{namespace}.{name}")
    if missing:
        fail(f"windows_common missing SetLastError Bool ABI checked wrapper(s): {missing[:20]}")
...
def check_required_multiple_success_bool_status_returns(
    ...
    if invalid:
        fail(f"windows_common invalid multi-success BOOL native helper return(s): {invalid[:20]}")
...
def check_required_set_last_error_bool_false_success_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError Bool false-success checked wrapper(s)",
        "windows_common invalid SetLastError Bool false-success checked wrapper(s)",
    )
...
def check_required_set_last_error_bool_status_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError Bool status checked wrapper(s)",
        "windows_common invalid SetLastError Bool status checked wrapper(s)",
    )
...
def check_required_set_last_error_handle_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError null-handle ABI checked wrapper(s)",
        "windows_common invalid SetLastError null-handle ABI checked wrapper(s)",
    )
...
def check_required_set_last_error_invalid_handle_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError invalid-handle ABI checked wrapper(s)",
        "windows_common invalid SetLastError invalid-handle ABI checked wrapper(s)",
    )
...
def check_required_set_last_error_null_success_handle_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError null-success handle ABI checked wrapper(s)",
        "windows_common invalid SetLastError null-success handle ABI checked wrapper(s)",
    )
...
def check_required_set_last_error_wait_event_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError WAIT_EVENT checked wrapper(s)",
        "windows_common invalid SetLastError WAIT_EVENT checked wrapper(s)",
    )
...
def check_required_set_last_error_null_pointer_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError null-pointer checked wrapper(s)",
        "windows_common invalid SetLastError null-pointer checked wrapper(s)",
    )
...
def check_required_set_last_error_tls_value_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError TLS value checked wrapper(s)",
        "windows_common invalid SetLastError TLS value checked wrapper(s)",
    )
...
def check_required_set_last_error_uint32_minus_one_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError UInt32 -1 checked wrapper(s)",
        "windows_common invalid SetLastError UInt32 -1 checked wrapper(s)",
    )
...
def check_required_set_last_error_uint32_zero_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError UInt32 zero checked wrapper(s)",
        "windows_common invalid SetLastError UInt32 zero checked wrapper(s)",
    )
...
def check_required_set_last_error_uint32_zero_last_error_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError UInt32 zero-last-error checked wrapper(s)",
        "windows_common invalid SetLastError UInt32 zero-last-error checked wrapper(s)",
    )
...
def check_required_set_last_error_uintnative_zero_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError UIntNative zero checked wrapper(s)",
        "windows_common invalid SetLastError UIntNative zero checked wrapper(s)",
    )
...
def check_required_set_last_error_intnative_zero_last_error_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError IntNative zero-last-error checked wrapper(s)",
        "windows_common invalid SetLastError IntNative zero-last-error checked wrapper(s)",
    )
...
def check_required_set_last_error_int32_max_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError Int32 max checked wrapper(s)",
        "windows_common invalid SetLastError Int32 max checked wrapper(s)",
    )
...
def check_required_set_last_error_int32_minus_one_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing SetLastError Int32 -1 checked wrapper(s)",
        "windows_common invalid SetLastError Int32 -1 checked wrapper(s)",
    )
...
def check_required_winsock_invalid_socket_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing WinSock INVALID_SOCKET checked wrapper(s)",
        "windows_common invalid WinSock INVALID_SOCKET checked wrapper(s)",
    )
...
def check_required_winsock_bool_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing WinSock BOOL checked wrapper(s)",
        "windows_common invalid WinSock BOOL checked wrapper(s)",
    )
...
def check_required_winsock_wait_event_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing WinSock WAIT_EVENT checked wrapper(s)",
        "windows_common invalid WinSock WAIT_EVENT checked wrapper(s)",
    )
...
def check_required_winsock_invalid_event_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing WinSock WSA_INVALID_EVENT checked wrapper(s)",
        "windows_common invalid WinSock WSA_INVALID_EVENT checked wrapper(s)",
    )
...
def check_required_winsock_null_handle_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing WinSock null-handle checked wrapper(s)",
        "windows_common invalid WinSock null-handle checked wrapper(s)",
    )
...
def check_required_winsock_null_pointer_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing WinSock null-pointer checked wrapper(s)",
        "windows_common invalid WinSock null-pointer checked wrapper(s)",
    )
...
def check_required_winsock_socket_error_int32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing WinSock SOCKET_ERROR Int32 checked wrapper(s)",
        "windows_common invalid WinSock SOCKET_ERROR Int32 checked wrapper(s)",
    )
...
def check_required_mswinsock_get_last_error_socket_error_int32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing MSWSOCK GetLastError SOCKET_ERROR Int32 checked wrapper(s)",
        "windows_common invalid MSWSOCK GetLastError SOCKET_ERROR Int32 checked wrapper(s)",
    )
...
def check_required_winsock_direct_error_int32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing WinSock direct-error Int32 checked wrapper(s)",
        "windows_common invalid WinSock direct-error Int32 checked wrapper(s)",
    )
...
def check_required_winsock_pending_status_int32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing WinSock pending-status Int32 checked wrapper(s)",
        "windows_common invalid WinSock pending-status Int32 checked wrapper(s)",
    )
...
def check_required_winsock_lperrno_int32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing WinSock lpErrno Int32 checked wrapper(s)",
        "windows_common invalid WinSock lpErrno Int32 checked wrapper(s)",
    )
...
def check_required_wait_failed_uint32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing WAIT_FAILED UInt32 checked wrapper(s)",
        "windows_common invalid WAIT_FAILED UInt32 checked wrapper(s)",
    )
...
def check_required_direct_win32_error_code_int32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing direct Win32 error-code Int32 checked wrapper(s)",
        "windows_common invalid direct Win32 error-code Int32 checked wrapper(s)",
    )
...
def check_required_direct_win32_error_code_uint32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing direct Win32 error-code UInt32 checked wrapper(s)",
        "windows_common invalid direct Win32 error-code UInt32 checked wrapper(s)",
    )
...
def check_required_uint32_less_than_or_equal_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing UInt32 <= threshold checked wrapper(s)",
        "windows_common invalid UInt32 <= threshold checked wrapper(s)",
    )
...
def check_required_hresult_com_out_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing HRESULT COM out ABI checked wrapper(s)",
        "windows_common invalid HRESULT COM out ABI checked wrapper(s)",
...
def check_required_hresult_owned_string_out_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing HRESULT owned string out ABI checked wrapper(s)",
        "windows_common invalid HRESULT owned string out ABI checked wrapper(s)",
        uninspectable_message="{namespace}.{name}Checked must return windows_core.Result<Unit>",
    )
...
def check_required_hresult_owned_pointer_out_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing HRESULT owned pointer out ABI checked wrapper(s)",
        "windows_common invalid HRESULT owned pointer out ABI checked wrapper(s)",
        uninspectable_message="{namespace}.{name}Checked must return windows_core.Result<Unit>",
    )
...
def check_required_hresult_borrowed_optional_out_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing HRESULT borrowed optional out ABI checked wrapper(s)",
        "windows_common invalid HRESULT borrowed optional out ABI checked wrapper(s)",
        uninspectable_message="{namespace}.{name}Checked must return windows_core.Result<Unit>",
    )
...
def check_required_hresult_optional_scalar_out_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing HRESULT optional scalar out ABI checked wrapper(s)",
        "windows_common invalid HRESULT optional scalar out ABI checked wrapper(s)",
        uninspectable_message="{namespace}.{name}Checked must return windows_core.Result<Unit>",
    )
...
def check_required_multiple_success_hresult_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        "windows_common missing multiple-success HRESULT checked wrapper(s)",
        "windows_common invalid multiple-success HRESULT checked wrapper(s)",
...
def check_package_boundaries(workspace: Path) -> None:
    ...
    common_com_helpers = workspace / "windows_common" / "src" / "Win32" / "System" / "Com" / "native_helpers.cj"
    if common_com_helpers.exists():
        ...
        for pattern in (
            ...
            if re.search(pattern, common_com_text):
                fail("windows_common COM native helpers must preserve pointer-to-pointer out slots")
    ...
    app_runtime_helpers = workspace / "windows_common" / "src" / "Native" / "AppRuntime" / "native_helpers.cj"
    check_path_exists(app_runtime_helpers, "windows_common AppRuntime native helpers")
    app_runtime_text = app_runtime_helpers.read_text(encoding="utf-8")
    if "resolveProc(bootstrapDllPath" in app_runtime_text:
        fail("windows_common AppRuntime helpers must use the explicit search-path resolver")
    if app_runtime_text.count("resolveProcFromSearchPath(bootstrapDllPath") < 2:
        fail("windows_common AppRuntime helpers must resolve bootstrap exports via the explicit search-path resolver")
    if "MddBootstrapInitialize2" not in app_runtime_text or "MddBootstrapShutdown" not in app_runtime_text:
        fail("windows_common AppRuntime helpers must include both bootstrap exports")
    common_com_helpers = workspace / "windows_common" / "src" / "Win32" / "System" / "Com" / "native_helpers.cj"
    ...
    if "public func CoTaskMemAlloc(cb: UIntNative): CPointer<Unit>" not in common_com_helpers_text:
        fail("windows_common CoTaskMemAlloc helper must expose SIZE_T byte count as UIntNative")
    if "public func CoTaskMemRealloc(pv: CPointer<Unit>, cb: UIntNative): CPointer<Unit>" not in common_com_helpers_text:
        fail("windows_common CoTaskMemRealloc helper must expose SIZE_T byte count as UIntNative")
...
def main() -> None:
    ...
    print("OK: generated windows_common package is present and marked DO NOT EDIT")
