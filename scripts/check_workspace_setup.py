...
# / windows_future; each keeps a smoke file that exercises a real WinRT path so
...
GENERATED_TOML_HEADER = "# Generated by windows_bindgen. DO NOT EDIT."
GENERATED_MANIFEST_HEADER = windows_sys_manifest.GENERATED_MANIFEST_HEADER
GENERATED_COMMON_IGNORED_FILES = windows_sys_manifest.IGNORED_GENERATED_FILES
...
FINALIZER_SAFE_CLEANUP_HELPERS = {
    ...
    "closeRegistryKeyNoThrow",
    "destroySafeArrayNoThrow",
    "releasePropVariantStorageNoThrow",
    "releaseVariantStorageNoThrow",
}
...
Win32MetadataPayloads = tuple[list[dict], list[str]]
Win32TypeDef = tuple[str, dict]
NativeHelpersByNamespace = dict[str, tuple[set[str], set[str]]]
CheckedWrappersByNamespace = dict[str, dict[str, tuple[str, str, int]]]
NativeWrapperRuleMatcher = Callable[[dict], object | None]
NativeWrapperRuleIssueFinder = Callable[[str, str, object], Sequence[str]]
...
def read_bindgen_tests_text(workspace: Path) -> str:
    return read_tests_text(workspace / "windows_bindgen" / "src")
...
def check_winui_xaml_failed_out_cleanup(workspace: Path) -> None:
    ...
    check_path_exists(xaml, "windows_winui3 XAML helper")
    text = xaml.read_text(encoding="utf-8")
    require_text_before(
        text,
        "releaseFailedWinuiOutSlot(CPointer<CPointer<Unit>>(inout inner))",
        'checkHRESULT(hr, "IWindowFactory.CreateInstance")',
        "windows_winui3 IWindowFactory.CreateInstance must release failed inner out slot before checkHRESULT",
    )
    require_text_before(
        text,
        "releaseFailedWinuiOutSlot(CPointer<CPointer<Unit>>(inout result))",
        'checkHRESULT(hr, "IWindowFactory.CreateInstance")',
        "windows_winui3 IWindowFactory.CreateInstance must release failed result out slot before checkHRESULT",
    )
    require_text_before(
        text,
        "releaseFailedWinuiOutSlot(CPointer<CPointer<Unit>>(inout result))",
        'checkHRESULT(hr, "IFrameworkElement.FindName(${name})")',
        "windows_winui3 IFrameworkElement.FindName must release failed result out slot before checkHRESULT",
    )
    require_text_before(
        text,
        "releaseFailedWinuiOutSlot(CPointer<CPointer<Unit>>(inout result))",
        'checkHRESULT(hr, "IXamlReaderStatics.Load")',
        "windows_winui3 IXamlReaderStatics.Load must release failed result out slot before checkHRESULT",
    )
    cleanup = section_between(
        text,
        "func releaseFailedWinuiOutSlot",
        "func readWinuiVtbl",
        "windows_winui3 failed out-slot cleanup helper",
    )
    require_text_before(
        cleanup,
        "unsafe { slot.write(CPointer<Unit>()) }",
        "releaseRaw(raw)",
        "windows_winui3 failed out-slot cleanup must clear the slot before releasing it",
    )
...
def check_winui_xaml_callback_com_invariants(workspace: Path) -> None:
    ...
    button_vtbl = section_between(
        ...
        "struct WindowVtbl",
        "windows_winui3 default ButtonBaseVtbl",
    )
    require_text_fragments(
        ...
        "windows_winui3 default ButtonBaseVtbl.AddClick must clear stale token out slots before E_NOTIMPL",
    )
    require_text_before(
        button_vtbl,
        "unsafe { result.write(0) }",
    ...
    routed_default_vtbl = section_between(
        ...
        "windows_winui3 default RoutedEventHandlerVtbl",
    )
    require_text_fragments(
        routed_default_vtbl,
        (
        ...
        "windows_winui3 default RoutedEventHandlerVtbl.QueryInterface must clear stale result out slots before E_NOTIMPL",
    )
    require_text_before(
        routed_default_vtbl,
        "unsafe { result.write(CPointer<Unit>()) }",
    ...
    for prefix, iid_name, clear_slot in callback_pairs:
        query = section_between(
            ...
            f"windows_winui3 {prefix} QueryInterface",
        )
        require_text_fragments(
            query,
            (
            ...
            f"windows_winui3 {prefix} QueryInterface must preserve COM out-pointer/AddRef semantics",
        )
        pointer_failure = section_between(
            query,
            "if (thisPtr.isNull() || iidPtr.isNull() || result.isNull())",
            "let iid = unsafe { iidPtr.read() }",
            f"windows_winui3 {prefix} QueryInterface pointer-failure branch",
        )
        require_text_fragments(
            pointer_failure,
            (
            ...
            f"windows_winui3 {prefix} QueryInterface must only clear E_POINTER result slots after a null-result guard",
        )
        require_text_before(
            pointer_failure,
            "if (result.isNotNull())",
            "unsafe { result.write(CPointer<Unit>()) }",
            f"windows_winui3 {prefix} QueryInterface must guard E_POINTER out-slot clearing",
        )
        require_text_before(
            pointer_failure,
            "unsafe { result.write(CPointer<Unit>()) }",
            "return E_POINTER.value",
            f"windows_winui3 {prefix} QueryInterface must clear failed out pointers before E_POINTER",
        )
        require_text_before(
            query,
            "unsafe { result.write(CPointer<Unit>()) }",
            "return E_NOINTERFACE.value",
            f"windows_winui3 {prefix} QueryInterface must clear failed out pointers before E_NOINTERFACE",
        )
        ...
        release = section_between(
            ...
            f"windows_winui3 {prefix} Release",
        )
        require_text_fragments(
            release,
            (
            ...
            f"windows_winui3 {prefix} Release must clear slots and free exactly on final Release",
        )
        require_text_before(
            release,
            "unsafe { obj.write(value) }",
            "coTaskMemFree(thisPtr)",
            f"windows_winui3 {prefix} Release must persist cleared slot before freeing storage",
        )
        ...
        invoke = section_between(
            ...
            f"func {prefix}Invoke",
            f"func {prefix}Vtbl",
            f"windows_winui3 {prefix} Invoke",
        )
        require_text_fragments(
            invoke,
            (
            ...
            f"windows_winui3 {prefix} Invoke must preserve WindowsException HRESULTs before generic failure mapping",
        )
        require_text_before(
            invoke,
            "catch (error: WindowsException)",
            "catch (err: Exception)",
            f"windows_winui3 {prefix} Invoke must catch WindowsException before generic Exception",
        )
        require_text_before(
            invoke,
            "return error.code().value",
            "return E_FAIL.value",
            f"windows_winui3 {prefix} Invoke must return the original WindowsException HRESULT before generic E_FAIL",
        )
    ...
    check_hresult = section_between(
        ...
        "func checkHRESULT",
        "func expectFactory",
        "windows_winui3 checkHRESULT",
    )
    require_text_fragments(
        check_hresult,
        ("throw WindowsException(hr)",),
        "windows_winui3 checkHRESULT must throw WindowsException with the original HRESULT",
    )
    if "throw Exception(" in check_hresult:
        fail("windows_winui3 checkHRESULT must not collapse HRESULT failures into plain Exception")


def check_pinvoke_string_out_cleanup_gate(workspace: Path) -> None:
    render_symbol = workspace / "windows_bindgen" / "src" / "render_symbol.cj"
    check_path_exists(render_symbol, "windows_bindgen render_symbol")
    ...
    require_text_fragments(
        ...
        "windows_bindgen P/Invoke wrappers must release failed non-retval string handle out slots",
    )
    ...
    require_text_fragments(
        ...
        "windows_bindgen tests must cover failed non-retval HSTRING/BSTR out cleanup",
    )


def check_native_hresult_optional_out_cleanup_gate(workspace: Path) -> None:
    native_helpers = workspace / "windows_bindgen" / "src" / "native_helpers.cj"
    check_path_exists(native_helpers, "windows_bindgen native helpers")
    ...
    require_text_fragments(
        ...
        "windows_bindgen native HRESULT wrappers must model optional owned/borrowed/scalar out slots",
    )
    test_text = read_bindgen_tests_text(workspace)
    require_text_fragments(
        test_text,
        ...
        "windows_bindgen tests must cover CoQuery*Blanket and RTWorkQ optional out cleanup",
    )
...
def check_interface_dispatcher_exception_out_cleanup(workspace: Path) -> None:
    ...
    require_text_fragments(
        ...
        "windows_bindgen interface dispatchers must release partial COM/HSTRING/array out slots on thrown exceptions",
    )
    require_text_before(
        render_text,
        "interfaceDispatcherReleaseFailedOutSlots(cleanupSpecs, indentLevel + 1)",
        "interfaceDispatcherClearPairs(outPairs, indentLevel + 1)",
        "windows_bindgen interface dispatcher catch paths must release partial out slots before clearing them",
    )
    test_text = read_bindgen_tests_text(workspace)
    require_text_fragments(
        test_text,
        ...
        "windows_bindgen tests must cover interface dispatcher partial COM/HSTRING/array out cleanup",
    )


def check_generated_interface_dispatcher_failed_hresult_out_cleanup(workspace: Path) -> None:
    impl_root = workspace / "windows_sys" / "src" / "impl"
    check_path_exists(impl_root, "windows_sys generated impl directory")
    sources = sorted(impl_root.glob("symbols_*.cj"))
    if not sources:
        fail("windows_sys generated impl directory must contain symbols_*.cj")
...
def check_runtime_class_raw_abi_array_out_preclear(workspace: Path) -> None:
    ...
    require_text_fragments(
        ...
        "windows_bindgen runtime-class projected wrappers must pre-clear sibling OutSlot values before E_POINTER",
    )
    ...
    require_text_fragments(
        ...
        "windows_bindgen runtime-class raw ABI wrappers must pre-clear array out slots before dispatch",
    )
    ...
    require_text_fragments(
        ...
        "windows_bindgen runtime-class raw ABI wrappers must retain failed-HRESULT array out-slot cleanup after dispatch",
    )
    ...
    require_text_before(
        ...
        "windows_bindgen runtime-class factory raw ABI wrappers must pre-clear array out slots before factory__ dispatch",
    )
    require_text_before(
        factory_body,
        "hr__ = unsafe { factory__.${methodName}(${args}) }",
        "renderRuntimeClassRawAbiFailureCleanup(method, symbolsByName, symbol.fullName, 2)",
        "windows_bindgen runtime-class factory raw ABI wrappers must keep failed-HRESULT cleanup after factory__ dispatch",
    )
    ...
    require_text_before(
        ...
        "windows_bindgen runtime-class interface raw ABI wrappers must pre-clear array out slots before interface__ dispatch",
    )
    require_text_before(
        instance_body,
        "hr__ = unsafe { interface__.",
        "renderRuntimeClassRawAbiFailureCleanup(method, symbolsByName, symbol.fullName, 2)",
        "windows_bindgen runtime-class interface raw ABI wrappers must keep failed-HRESULT cleanup after interface__ dispatch",
    )
    ...
    require_text_fragments(
        ...
        "windows_bindgen tests must mark runtime-class raw ABI array out-slot pre-clear before raw ABI calls",
    )


def check_test_com_object_owners_released(workspace: Path) -> None:
    source_roots = [
        workspace / "windows_foundation" / "src",
        workspace / "windows_collections" / "src",
        workspace / "windows_future" / "src",
        workspace / "windows_interface" / "tests" / "macros",
    ]
...
def check_temporary_com_object_projection_ownership(workspace: Path) -> None:
    source_roots = [
        ...
        workspace / "windows_collections" / "src",
        workspace / "windows_future" / "src",
        workspace / "windows_implement" / "src",
        workspace / "windows_threading" / "src",
    ]
    for source_root in source_roots:
        if not source_root.exists():
            continue
...
def check_threading_pools_use_scope_or_explicit_close(workspace: Path) -> None:
    ...
    source_roots = [
        workspace / "windows_core" / "src",
        workspace / "windows_implement" / "src",
        workspace / "windows_foundation" / "src",
        workspace / "windows_collections" / "src",
        workspace / "windows_future" / "src",
        workspace / "windows_services" / "src",
        workspace / "windows_threading" / "src",
    ]
    for source_root in source_roots:
        if not source_root.exists():
        ...
        for source in source_root.rglob("*.cj"):
            ...
            if source.name.endswith("_test.cj"):
                continue
            if relative.as_posix() == "windows_threading/src/lib.cj":
                continue
            lines = source.read_text(encoding="utf-8").splitlines()
            for index, line in enumerate(lines):
...
def check_windows_string_finalizer_paths_are_lock_free(workspace: Path) -> None:
    strings_root = workspace / "windows_strings" / "src"
    ref_count = strings_root / "ref_count.cj"
    native = strings_root / "native.cj"
    check_path_exists(ref_count, "windows_strings HString ref-count implementation")
    check_path_exists(native, "windows_strings native ABI implementation")
...
def check_long_lived_vtables_are_unmanaged(workspace: Path) -> None:
    ...
    source_roots = [
        workspace / "windows_core" / "src",
        workspace / "windows_implement" / "src",
        workspace / "windows_interface" / "src",
        workspace / "windows_foundation" / "src",
        workspace / "windows_collections" / "src",
        workspace / "windows_future" / "src",
    ]
    ...
    for source_root in source_roots:
        ...
        for source in sorted(source_root.rglob("*.cj")):
            ...
            if any(part in IGNORED_SHELL_SCRIPT_DIRS for part in relative.parts):
                continue
            if relative.as_posix() == "windows_interface/src/macros/windows_interface_macros.cj":
                continue
            text = source.read_text(encoding="utf-8")
...
def check_collection_indexof_thunks_reject_null_index(workspace: Path) -> None:
    runtime_path = workspace / "windows_collections" / "src" / "collections_runtime.cj"
    text = runtime_path.read_text(encoding="utf-8")
    bodies = iter_cj_function_bodies(text, "__winrtThunk_IndexOf")
    if len(bodies) < 3:
        fail("windows_collections collection IndexOf thunks are missing expected producer implementations")
    for line, body in bodies:
        ...
        if has_index_guard is None:
            fail(
                f"windows_collections/src/collections_runtime.cj:{line} IndexOf thunk must reject null "
...
def check_generated_collection_vtable_abi_parity(workspace: Path) -> None:
    impl_root = workspace / "windows_sys" / "src" / "impl"
    check_path_exists(impl_root, "windows_sys generated impl sources")
    ...
    for label, fragment in forbidden_fragments.items():
        if fragment in impl_text:
            fail(f"windows_sys generated collection vtable ABI still uses {label}")
    for label, fragment in required_fragments.items():
        if fragment not in impl_text:
            fail(f"windows_sys generated collection vtable ABI missing {label}")
...
def check_collection_generic_input_thunks_translate_projection_failures(workspace: Path) -> None:
    ...
    for name in thunk_names:
        for line, body in iter_cj_function_bodies(text, f"__winrtThunk_{name}"):
            ...
            if "catch (error: windows_core.WindowsException)" not in body or "windows_core.E_FAIL.value" not in body:
                fail(
                    f"windows_collections/src/collections_runtime.cj:{line} {name} thunk must translate "
            ...
            if "CPointer<Bool>" in body and "clearCollectionBoolOutSlot(result__)" not in prefix:
                fail(
                    f"windows_collections/src/collections_runtime.cj:{line} {name} thunk must clear Bool "
                    "out slots before projecting generic inputs"
                )
            if name == "IndexOf":
                if "clearCollectionIndexOutSlot(arg1)" not in prefix:
                    fail(
                        f"windows_collections/src/collections_runtime.cj:{line} IndexOf thunk must clear "
    ...
    if checked < 18:
        fail("windows_collections collection generic input thunk audit saw too few guarded thunks")


def check_collection_scalar_output_thunks_clear_outputs(workspace: Path) -> None:
    runtime_path = workspace / "windows_collections" / "src" / "collections_runtime.cj"
    test_path = workspace / "windows_collections" / "src" / "collection_get_many_cleanup_test.cj"
    default_vtable_test_path = workspace / "windows_collections" / "src" / "collection_default_vtable_abi_test.cj"
    null_buffer_test_path = workspace / "windows_collections" / "src" / "collection_null_buffer_thunk_test.cj"
    text = runtime_path.read_text(encoding="utf-8")
    require_text_fragments(
        text,
        (
            ...
            "windows_core.foundationUnitThunk(action)",
        ...
        "windows_collections collection manual thunks must use shared cleanup/translation helpers",
    )
    ...
    require_text_before(
        ...
        "windows_collections collection GetMany helper must clear written out slots before E_POINTER buffer failures",
    )
    ...
    if split_failure_start < 0 or split_failure_return < 0:
        fail("windows_collections collection Split helper must reject null out pointers with E_POINTER")
    ...
    require_text_fragments(
        ...
        "windows_collections collection Split helper must clear non-null sibling out slots before E_POINTER",
    )
    specs = (
    ...
    for name, dispatch_fragment, clear_fragment, minimum in specs:
        ...
        for line, body in iter_cj_function_bodies(text, f"__winrtThunk_{name}"):
            ...
            if clear_fragment not in prefix:
                fail(
                    f"windows_collections/src/collections_runtime.cj:{line} {name} thunk must clear "
        ...
        if checked < minimum:
            fail(f"windows_collections collection scalar thunk audit saw too few {name} thunks")

    check_path_exists(test_path, "windows_collections collection scalar thunk cleanup tests")
    ...
    require_text_fragments(
        ...
        "windows_collections collection scalar output thunk cleanup tests must cover stale slots and exceptions",
    )
    check_path_exists(default_vtable_test_path, "windows_collections collection default vtable ABI tests")
    ...
    require_text_fragments(
        ...
        "windows_collections collection default vtable ABI tests must cover E_NOTIMPL out-slot clearing",
    )
    check_path_exists(null_buffer_test_path, "windows_collections collection null-buffer thunk tests")
    ...
    require_text_fragments(
        ...
        "windows_collections collection null-pointer thunk tests must cover E_POINTER sibling out-slot clearing",
    )
...
def check_iinspectable_get_iids_clears_sibling_out_slots(workspace: Path) -> None:
    ...
    thunk_builder = workspace / "windows_implement" / "src" / "thunk_builder.cj"
    interface_surface = workspace / "windows_implement" / "src" / "interface_impl_surface.cj"
    core_inspectable = workspace / "windows_core" / "src" / "inspectable.cj"
    implement_tests = workspace / "windows_implement" / "src" / "com_object_lifetime_test.cj"
    interface_tests = workspace / "windows_interface" / "src" / "descriptor_basics_test.cj"
    core_tests = workspace / "windows_core" / "src" / "core_descriptor_test.cj"
    for path, label in (
        (interface_wrapper, "windows_interface interface wrapper"),
        (thunk_builder, "windows_implement thunk builder"),
        (interface_surface, "windows_implement interface surface"),
        (core_inspectable, "windows_core inspectable thunks"),
        (implement_tests, "windows_implement COM object lifetime tests"),
        (interface_tests, "windows_interface descriptor basics tests"),
        (core_tests, "windows_core descriptor tests"),
    ):
    ...
    iunknown_vtbl_section = section_between(
        ...
        "windows_interface default IUnknownVtbl",
    )
    query_interface_section = section_between(
        iunknown_vtbl_section,
        "public var QueryInterface",
        "public var AddRef",
        "windows_interface default IUnknownVtbl.QueryInterface",
    )
    require_text_fragments(
        ...
        "windows_interface default IUnknownVtbl.QueryInterface must reject null result slots and clear stale out pointers",
    )
    require_text_before(
        query_interface_section,
        "if (resultPtr.isNull())",
        "return E_POINTER.value",
        "windows_interface default IUnknownVtbl.QueryInterface must reject null result slots",
    )
    require_text_before(
        query_interface_section,
        "resultPtr.write(CPointer<Unit>())",
        "E_NOINTERFACE.value",
        "windows_interface default IUnknownVtbl.QueryInterface must clear failed out pointers before E_NOINTERFACE",
    )
    require_text_fragments(
        wrapper_text,
        (
        ...
        "windows_interface default unsupported vtable stubs must have failed out-slot clear helpers",
    )
    unsupported_out_slot_methods = (
        (
            ...
            "windows_interface default IAgileReferenceVtbl.Resolve",
        ),
        (
            "public struct IMarshalVtbl",
            ...
            "windows_interface default IMarshalVtbl.GetUnmarshalClass",
        ),
        (
            "public struct IMarshalVtbl",
            "public struct IWeakReferenceVtbl",
            ...
            "windows_interface default IMarshalVtbl.GetMarshalSizeMax",
        ),
        (
            "public struct IMarshalVtbl",
            "public struct IWeakReferenceVtbl",
            ...
            "clearDefaultObjectOutSlot(resultPtr)",
            "windows_interface default IMarshalVtbl.UnmarshalInterface",
        ),
        (
            "public struct IWeakReferenceVtbl",
            ...
            "public init()",
            "clearDefaultObjectOutSlot(resultPtr)",
            "windows_interface default IWeakReferenceVtbl.Resolve",
        ),
        (
            "public struct IWeakReferenceSourceVtbl",
            ...
            "public init()",
            "clearDefaultObjectOutSlot(resultPtr)",
            "windows_interface default IWeakReferenceSourceVtbl.GetWeakReference",
        ),
        (
            ...
            "public init()",
            "clearDefaultObjectOutSlot(resultPtr)",
            "windows_interface default IActivationFactoryVtbl.ActivateInstance",
        ),
    )
    ...
    require_text_fragments(
        ...
        "windows_interface default IInspectableVtbl.GetIids must clear non-null sibling out slots before E_POINTER",
    )
    ...
    require_text_fragments(
        ...
        "windows_implement clearInspectableIidsOutSlots must clear non-null GetIids out slots",
    )
    require_text_fragments(
        thunk_body,
        (
        ...
        "windows_implement iinspectableGetIidsThunk must clear stale outputs and translate thrown exceptions",
    )
    ...
    if thunk_iids_cleanup_count < 4:
        fail("windows_implement iinspectableGetIidsThunk must clear outputs on pointer failure, before dispatch, and in both catch paths")
    iids_dispatch_index = thunk_body.find("inspectable.getIidsBase(count, values)")
    if iids_dispatch_index < 0:
        fail("windows_implement iinspectableGetIidsThunk must dispatch to IInspectableBase.getIidsBase")
    if thunk_body[:iids_dispatch_index].count("clearInspectableIidsOutSlots(count, values)") < 2:
        fail("windows_implement iinspectableGetIidsThunk must clear valid out slots before dispatch")
    require_iinspectable_iids_release_helper_call(
        thunk_body,
        iids_release_helper,
        "inspectable.getIidsBase(count, values)",
        "windows_implement iinspectableGetIidsThunk",
    )
    ...
    require_text_fragments(
        ...
        "windows_core coreIInspectableGetIidsThunk must translate thrown exceptions",
    )
    require_iinspectable_iids_release_helper_call(
        core_thunk_body,
        core_release_helper,
        "value.getIidsBase(count, values)",
        "windows_core coreIInspectableGetIidsThunk",
    )
    ...
    require_text_fragments(
        ...
        "windows_core coreIInspectableGetRuntimeClassNameThunk must translate thrown exceptions and release partial HSTRING outputs",
    )
    ...
    require_text_fragments(
        ...
        "windows_core clearInspectableTrustLevelOutSlot must clear non-null GetTrustLevel out slots",
    )
    require_text_fragments(
        ...
        "windows_core coreIInspectableGetTrustLevelThunk must clear stale outputs and translate thrown exceptions",
    )
    if core_trust_thunk_body.count("clearInspectableTrustLevelOutSlot(value)") < 3:
        fail("windows_core coreIInspectableGetTrustLevelThunk must clear outputs before dispatch and in both catch paths")
    ...
    require_text_fragments(
        ...
        "windows_implement iinspectableGetRuntimeClassNameThunk must translate thrown exceptions and release partial HSTRING outputs",
    )
    ...
    require_text_fragments(
        trust_clear_helper,
        ("if (value.isNotNull())", "value.write(0i32)"),
        "windows_implement clearTrustLevelOutSlot must clear non-null GetTrustLevel out slots",
    )
    require_text_fragments(
        trust_thunk_body,
        (
        ...
        "windows_implement iinspectableGetTrustLevelThunk must clear stale outputs and translate thrown exceptions",
    )
    if trust_thunk_body.count("clearTrustLevelOutSlot(value)") < 3:
        fail("windows_implement iinspectableGetTrustLevelThunk must clear outputs before dispatch and in both catch paths")
    ...
    if not surface_bodies:
        fail("windows_implement ComObjectRuntime.getIidsBase must be present")
    ...
    if failure_start < 0 or failure_return < 0:
        fail("windows_implement ComObjectRuntime.getIidsBase must reject null GetIids out pointers")
    ...
    require_text_fragments(
        ...
        "windows_implement ComObjectRuntime.getIidsBase must clear non-null sibling out slots before E_POINTER",
    )
    ...
    require_text_fragments(
        ...
        "windows_implement tests must cover IInspectable thunk stale out-slot clearing and exception translation",
    )
    ...
    require_text_fragments(
        ...
        "windows_interface tests must cover default IUnknown/IInspectable vtable out-slot clearing",
    )
    ...
    require_text_fragments(
        ...
        "windows_core tests must cover core IInspectable partial out cleanup and exception translation",
    )


def check_default_vtable_stubs_clear_failed_out_slots(workspace: Path) -> None:
    core_factory = workspace / "windows_core" / "src" / "generic_factory.cj"
    core_factory_tests = workspace / "windows_core" / "src" / "generic_factory_lifetime_test.cj"
    result_bindings = workspace / "windows_result" / "src" / "bindings.cj"
    result_tests = workspace / "windows_result" / "src" / "internal_test.cj"
    for path, label in (
        (core_factory, "windows_core generic factory"),
        (core_factory_tests, "windows_core generic factory tests"),
        (result_bindings, "windows_result bindings"),
        (result_tests, "windows_result internal tests"),
    ):
        check_path_exists(path, label)
    ...
    factory_vtbl_section = section_between(
        ...
        "windows_core default IGenericFactoryVtbl",
    )
    factory_activate_section = section_between(
        factory_vtbl_section,
        "public var ActivateInstance",
        "public init()",
        "windows_core default IGenericFactoryVtbl.ActivateInstance",
    )
    require_text_fragments(
        ...
        "windows_core default IGenericFactoryVtbl.ActivateInstance must clear stale result slots",
    )
    require_text_before(
        factory_activate_section,
        "result.write(CPointer<Unit>())",
        "E_NOTIMPL.value",
        "windows_core default IGenericFactoryVtbl.ActivateInstance must clear stale result slots before E_NOTIMPL",
    )
    ...
    require_text_fragments(
        ...
        "windows_core tests must cover default IGenericFactoryVtbl failed out-slot clearing",
    )
    ...
    require_text_fragments(
        ...
        "windows_result default COM error vtables must have failed out-slot clear helpers",
    )
    result_iunknown_section = section_between(
        result_bindings_text,
        "struct IUnknownVtbl",
        "@C\nstruct IErrorInfoVtbl",
        "windows_result default IUnknownVtbl",
    )
    result_query_section = section_between(
        result_iunknown_section,
        "var QueryInterface",
        "var AddRef",
        "windows_result default IUnknownVtbl.QueryInterface",
    )
    require_text_fragments(
        ...
        "windows_result default IUnknownVtbl.QueryInterface must reject null result slots and clear stale outputs",
    )
    require_text_before(
        result_query_section,
        "clearDefaultUnknownOutSlot(resultPtr)",
        "E_NOINTERFACE.value",
        "windows_result default IUnknownVtbl.QueryInterface must clear stale outputs before E_NOINTERFACE",
    )
    ...
    result_error_info_section = section_between(
        ...
        "windows_result default IErrorInfoVtbl",
    )
    error_info_methods = (
        ("var GetGUID", "var GetSource", "clearDefaultGuidOutSlot(result)", "windows_result default IErrorInfoVtbl.GetGUID"),
        ("var GetSource", "var GetDescription", "clearDefaultBstrOutSlot(result)", "windows_result default IErrorInfoVtbl.GetSource"),
        (
            "var GetDescription",
            "var GetHelpFile",
            "clearDefaultBstrOutSlot(result)",
            "windows_result default IErrorInfoVtbl.GetDescription",
        ),
        ("var GetHelpFile", "var GetHelpContext", "clearDefaultBstrOutSlot(result)", "windows_result default IErrorInfoVtbl.GetHelpFile"),
        ("var GetHelpContext", "init()", "clearDefaultUInt32OutSlot(result)", "windows_result default IErrorInfoVtbl.GetHelpContext"),
    )
    ...
    result_restricted_section = section_between(
        ...
        "struct IRestrictedErrorInfoVtbl",
        "func GetErrorInfo",
        "windows_result default IRestrictedErrorInfoVtbl",
    )
    details_section = section_between(
        result_restricted_section,
        "var GetErrorDetails",
        "var GetReference",
        "windows_result default IRestrictedErrorInfoVtbl.GetErrorDetails",
    )
    require_text_fragments(
        details_section,
        (
        ...
        "windows_result default IRestrictedErrorInfoVtbl.GetErrorDetails must clear all stale outputs",
    )
    reference_section = section_between(
        result_restricted_section,
        "var GetReference",
        "init()",
        "windows_result default IRestrictedErrorInfoVtbl.GetReference",
    )
    require_text_before(
        reference_section,
        "clearDefaultBstrOutSlot(result)",
        "E_NOTIMPL.value",
        "windows_result default IRestrictedErrorInfoVtbl.GetReference must clear stale outputs before E_NOTIMPL",
    )
    ...
    require_text_fragments(
        ...
        "windows_result tests must cover default COM error vtable failed out-slot clearing",
    )
...
def check_descriptor_generated_thunks_translate_exceptions(workspace: Path) -> None:
    ...
    check_path_exists(generator, "windows_implement descriptor code generator")
    check_path_exists(tests, "windows_implement descriptor codegen tests")
    ...
    require_text_fragments(
        ...
        "windows_implement descriptor generated thunks must have exception translation helpers",
    )
    require_text_before(
        append_body,
        'sb.append("    try {\\n")',
        'sb.append("    match (unsafe { asImplFromRaw<")',
        "windows_implement descriptor generated thunks must wrap ABI dispatch in try/catch",
    )
    require_text_fragments(
        append_body,
        (
        ...
        "windows_implement descriptor generated thunks must translate thrown exceptions to HRESULTs",
    )
    ...
    require_text_fragments(
        ...
        "windows_implement descriptor codegen tests must cover generated thunk exception translation",
    )
...
def check_collection_specialized_bool_thunks_clear_outputs(workspace: Path) -> None:
    ...
    for match in re.finditer(r"impl\.(HasKey|Insert)\s*\(", text):
        ...
        if "clearCollectionBoolOutSlot(result__)" not in prefix:
            line = text[: match.start()].count("\n") + 1
            fail(
                f"windows_collections/src/collections_runtime.cj:{line} specialized {match.group(1)} "
                "thunk must clear Bool out slots before dispatch"
            )
    if checked < 25:
        fail("windows_collections specialized collection Bool thunk audit saw too few vtable branches")
...
def check_collection_specialized_indexof_thunks_clear_outputs(workspace: Path) -> None:
    ...
    for match in pattern.finditer(text):
        ...
        if "result__.isNull() || arg1.isNull()" not in prefix:
            fail(
                f"windows_collections/src/collections_runtime.cj:{line} specialized IndexOf thunk must "
    ...
    if checked < 35:
        fail("windows_collections specialized collection IndexOf thunk audit saw too few vtable branches")
    ...
    for match in pattern.finditer(text):
        ...
        if "CPointer<CPointer<Unit>>" in signature:
            if "clearCollectionGenericOutSlot(result__)" not in prefix:
                fail(
                    f"windows_collections/src/collections_runtime.cj:{line} specialized Lookup thunk must "
            ...
            if "unsafe { result__.write(" not in prefix:
                fail(
                    f"windows_collections/src/collections_runtime.cj:{line} specialized scalar Lookup thunk "
                    "must clear value out slots before dispatch"
                )
    if checked < 20 or scalar_checked < 10:
        fail("windows_collections specialized collection Lookup thunk audit saw too few vtable branches")
...
def check_collection_specialized_vector_thunks_translate_exceptions(workspace: Path) -> None:
    ...
    for match in pattern.finditer(text):
        ...
        if (
            ...
            line = text[: match.start()].count("\n") + 1
            fail(
                f"windows_collections/src/collections_runtime.cj:{line} specialized vector {slot} "
                "thunk must translate thrown exceptions to HRESULT values before returning"
            )
    if checked < 80:
        fail("windows_collections specialized vector exception thunk audit saw too few vtable branches")


def check_collection_event_thunks_translate_exceptions_and_clear_tokens(workspace: Path) -> None:
    sources = [
        ("windows_collections/src/collections_runtime.cj", workspace / "windows_collections" / "src" / "collections_runtime.cj"),
        ("windows_foundation/src/foundation_runtime.cj", workspace / "windows_foundation" / "src" / "foundation_runtime.cj"),
    ]
    checked_add = 0
    checked_remove = 0
    ...
    if checked_add < 4 or checked_remove < 4:
        fail("windows_foundation/windows_collections event thunk audit saw too few event branches")
...
def check_generated_raw_event_add_wrappers_clear_tokens(workspace: Path) -> None:
    ...
    sources = sorted(impl_root.glob("symbols_*.cj"))
    if not sources:
        fail("windows_sys generated impl symbols files are missing")
    ...
    if checked < 2:
        fail("windows_sys generated raw event add wrapper audit saw too few wrappers")
...
def check_vtable_none_branches_clear_out_slots(workspace: Path) -> None:
    foundation = workspace / "windows_foundation" / "src" / "foundation_runtime.cj"
    collections = workspace / "windows_collections" / "src" / "collections_runtime.cj"
    ...
    winrt_abi_thunks = workspace / "windows_core" / "src" / "winrt_abi_thunks.cj"
    marshaler = workspace / "windows_core" / "src" / "marshaler.cj"
    core_agile_ref = workspace / "windows_core" / "src" / "agile_reference.cj"
    core_reference_ref = workspace / "windows_core" / "src" / "weak.cj"
    core_descriptor_tests = workspace / "windows_core" / "src" / "core_descriptor_test.cj"
    core_weak_ref = workspace / "windows_core" / "src" / "weak_ref_count.cj"
    implement_weak_ref = workspace / "windows_implement" / "src" / "weak_ref_count.cj"
    implement_surface_ref = workspace / "windows_implement" / "src" / "interface_impl_surface.cj"
    implement_composable_ref = workspace / "windows_implement" / "src" / "composable_activation.cj"
    implement_thunk_ref = workspace / "windows_implement" / "src" / "thunk_builder.cj"
    implement_weak_tests = workspace / "windows_implement" / "src" / "weak_ref_count_test.cj"
    interface_macro = workspace / "windows_interface" / "src" / "macros" / "windows_interface_macros.cj"
    interface_macro_fixture = workspace / "windows_interface" / "tests" / "macros" / "generated_interface_fixture.cj"
    bindgen_render = workspace / "windows_bindgen" / "src" / "render_symbol.cj"
    common_impl = workspace / "windows_sys" / "src" / "impl"
    check_path_exists(foundation, "windows_foundation foundation runtime")
    check_path_exists(collections, "windows_collections collections runtime")
    check_path_exists(marshaler, "windows_core marshaler runtime")
    check_path_exists(core_agile_ref, "windows_core agile reference runtime")
    check_path_exists(core_reference_ref, "windows_core weak reference interface runtime")
    check_path_exists(core_descriptor_tests, "windows_core descriptor tests")
    check_path_exists(core_weak_ref, "windows_core weak reference runtime")
    check_path_exists(implement_weak_ref, "windows_implement weak reference runtime")
    check_path_exists(implement_surface_ref, "windows_implement interface implementation surface")
    check_path_exists(implement_composable_ref, "windows_implement composable activation runtime")
    check_path_exists(implement_thunk_ref, "windows_implement COM thunk builder")
    check_path_exists(implement_weak_tests, "windows_implement weak reference tests")
    check_path_exists(interface_macro, "windows_interface Interface macro")
    check_path_exists(interface_macro_fixture, "windows_interface generated macro fixture")
    check_path_exists(bindgen_render, "windows_bindgen render_symbol")
    check_path_exists(common_impl, "windows_sys generated impl directory")
    check_path_exists(winrt_abi_thunks, "windows_core WinRT-ABI thunk helpers")
    ...
    require_text_fragments(
        ...
        "windows_core WinRT-ABI No-interface helpers must clear stale out slots",
    )
    ...
    require_text_fragments(
        ...
        "windows_foundation vtable None branches must clear stale out slots",
    )
    ...
    if value_raw_body is None:
        fail("windows_foundation IReferenceArray must expose ValueRaw")
    ...
    if value_raw_check < 0 or value_raw_release < 0 or value_raw_release > value_raw_check:
        fail("windows_foundation IReferenceArray.ValueRaw must release failed generic array out before HRESULT check")
    ...
    require_text_fragments(
        ...
        "windows_collections collection vtable None branches must clear stale out slots",
    )
    ...
    def require_marshaler_lookup_miss_cleanup(function_name: str, clear_fragment: str, out_name: str) -> None:
        ...
        if body is None:
            fail(f"windows_core marshaler must define {function_name}")
        branch_index = body.find("case None =>")
        if branch_index < 0:
            fail(f"windows_core marshaler {function_name} must handle lookup-miss None branch")
        ...
        if error_index < 0 or clear_index < 0 or clear_index > error_index:
            fail(
                f"windows_core marshaler {function_name} lookup-miss branch must clear "
    ...
    def require_marshaler_state_scalar_cleanup(
        ...
        if call_index < 0:
            fail(f"windows_core marshaler {function_name} must dispatch to inner marshaler")
        preclear_index = body.find(clear_fragment)
        if preclear_index < 0 or preclear_index > call_index:
            fail(f"windows_core marshaler {function_name} must clear {out_name} before inner dispatch")
        ...
        if failure_index < 0 or postclear_index < 0 or postclear_index < failure_index:
            fail(f"windows_core marshaler {function_name} must clear {out_name} after failed inner HRESULT")
    ...
    require_text_fragments(
        ...
        "windows_core weak/agile reference thunk tests must cover thrown-exception HRESULT translation",
    )
    ...
    def require_implement_qi_exception_translation(source: Path, text: str, function_name: str) -> None:
        if function_name == "queryInterfaceBase" and "match (queryInterfaceFallback)" in text:
            body = section_between(
                ...
                "case None => ()",
                "windows_implement QueryInterface fallback",
            )
        else:
            body = function_body(text, function_name)
            if body is None:
    ...
    require_text_fragments(
        ...
        "windows_implement QueryInterface fallback tests must cover thrown-exception HRESULT translation",
    )
    ...
    if activate_composable_body is None:
        fail("windows_implement composable activation runtime must define activateComposable")
    require_text_fragments(
        ...
        "windows_implement activateComposable must clean outer/base/inner ownership when activation throws",
    )
    if activate_composable_body.count("releaseComRaw(innerRaw)") < 2:
        fail("windows_implement activateComposable must release innerRaw in every thrown activation path")
    require_text_fragments(
        implement_weak_tests.read_text(encoding="utf-8"),
        (
        ...
        "windows_implement tests must cover activateComposable thrown-activation ownership cleanup",
    )
    ...
    require_text_fragments(
        ...
        "windows_interface @Interface macro vtable branches must clear likely out slots and translate exceptions",
    )
    failure_cleanup_body = function_body(interface_macro_text, "appendVtblFailureOutSlotCleanup")
    if failure_cleanup_body is None:
        fail("windows_interface @Interface macro appendVtblFailureOutSlotCleanup is missing")
    require_text_before(
        failure_cleanup_body,
        "appendVtblFailureArrayOutSlotCleanup(sb, method, param, elementProjectedType, indent)",
        "match (outSlotDefaultExpr(method, param))",
        "windows_interface @Interface macro must release array out slots before scalar clears can zero their size slots",
    )
    ...
    require_text_fragments(
        ...
        "windows_interface macro fixture must cover @Interface lookup-miss/live/failed-HRESULT out-slot cleanup",
    )
    ...
    if generic_vtbl_body is None:
        fail("windows_bindgen renderGenericWinrtInterfaceVtbl is missing")
    require_text_fragments(
        generic_vtbl_body,
        (
            "interfaceDefaultVtblStub(method, pairs, symbolsByName, symbol.fullName)",
        ),
        "windows_bindgen generic WinRT vtable defaults must use the default out-slot clearing stub",
    )
    require_text_fragments(
        bindgen_render_text,
        (
        ...
        "windows_bindgen default vtable stubs must clear generated out slots before E_NOTIMPL",
    )
    ...
    require_text_fragments(
        ...
        "windows_bindgen tests must cover default vtable E_NOTIMPL out-slot clearing",
    )
    ...
    require_text_fragments(
        ...
        "windows_sys generated vtable branches must clear representative out slots and translate thrown exceptions",
    )
    if (
        "try {\n                            impl.First(result)" not in common_text
        and "try {\n                            let resultHr__ = impl.First(result)" not in common_text
    ):
        fail("windows_sys generated vtable branches must dispatch representative First method inside try")
    for label, start, clear_fragments in (
        ...
        pointer_branch = section_between(
            ...
            start,
            "} else {",
            f"windows_sys generated {label} E_POINTER branch",
        )
        for clear_fragment in clear_fragments:
            require_text_before(
                ...
                clear_fragment,
                "windows_core.E_POINTER.value",
                f"windows_sys generated {label} E_POINTER branch must clear non-null out slots before E_POINTER",
            )
    for call in (
        "impl.First(result)",
        "impl.GetMany(itemsSize, items, result)",
        ...
        if call_index < 0:
            fail(f"windows_sys generated vtable branches must contain representative call {call}")
        ...
        if catch_index < 0:
            fail(f"windows_sys generated vtable branch for {call} must translate WindowsException")


def check_foundation_manual_thunks_translate_exceptions(workspace: Path) -> None:
    runtime_path = workspace / "windows_foundation" / "src" / "foundation_runtime.cj"
    check_path_exists(runtime_path, "windows_foundation foundation runtime")
    ...
    thunks_path = workspace / "windows_core" / "src" / "winrt_abi_thunks.cj"
    check_path_exists(thunks_path, "windows_core WinRT-ABI thunk helpers")
    async_runtime_path = workspace / "windows_future" / "src" / "async_runtime.cj"
    check_path_exists(async_runtime_path, "windows_future async runtime")
    text = runtime_path.read_text(encoding="utf-8")
    thunks_text = thunks_path.read_text(encoding="utf-8")
    async_text = async_runtime_path.read_text(encoding="utf-8")
    # Helper definitions now live in windows_core/src/winrt_abi_thunks.cj.
    require_text_fragments(
        thunks_text,
        (
            ...
            "func winrtReleaseGenericOutRangeNoThrow<T>",
        ...
        "windows_core WinRT-ABI thunk helpers must clear outputs and translate thrown exceptions",
    )
    ...
    require_text_fragments(
        ...
        "windows_foundation PropertyValue helpers must translate thrown exceptions",
    )
    ...
    require_text_fragments(
        ...
        "windows_core clearFoundationArrayOut must clear non-null array size/data out slots",
    )
    ...
    require_text_fragments(
        ...
        "windows_core clearFoundationUnitArrayOut must clear non-null array size/data out slots",
    )
    ...
    require_text_fragments(
        ...
        "windows_core IReferenceArray.Value helper must clear sibling out slots before E_POINTER",
    )
    require_text_before(
        reference_array_body,
        "clearFoundationUnitArrayOut(resultSize, result)",
    ...
    require_text_fragments(
        ...
        "windows_foundation IPropertyValue array prepare helper must clear sibling out slots before E_POINTER",
    )
    require_text_before(
        property_array_prepare_body,
        "clearFoundationArrayOut<T>(valueSize, value)",
    ...
    for name, helper_call in output_thunks.items():
        ...
        if body is None:
            fail(f"windows_future IAsyncInfo_Impl must define __winrtThunk_{name}")
        if helper_call not in body:
            fail(f"windows_future IAsyncInfo {name} thunk must use scalar helper cleanup/translation")
    ...
    for name in ("Cancel", "Close"):
        ...
        if f"foundationUnitThunk({{ => this.{name}() }})" not in body:
            fail(f"windows_future IAsyncInfo {name} thunk must use Unit helper exception translation")
    ...
    for fragment, minimum in (
        ...
        if (text.count(fragment) + async_text.count(fragment)) < minimum:
            fail("windows_foundation/windows_future thunks must use cleanup/translation helpers across inherited interfaces")

    property_cleanup_tests = workspace / "windows_foundation" / "src" / "property_value_array_thunk_cleanup_test.cj"
    ...
    check_path_exists(property_cleanup_tests, "windows_foundation property value cleanup tests")
    check_path_exists(property_abi_tests, "windows_foundation property value ABI tests")
    check_path_exists(default_vtable_tests, "windows_foundation foundation default vtable ABI tests")
    check_path_exists(async_default_vtable_tests, "windows_future async default vtable ABI tests")
    check_path_exists(async_tests, "windows_future async helper tests")
    check_path_exists(async_helpers, "windows_future async helper implementation")
    check_path_exists(memory_tests, "windows_foundation memory buffer thunk tests")
    require_text_fragments(
        ...
        "windows_foundation property value thunk cleanup tests must cover direct and inherited thrown-exception paths",
    )
    require_text_fragments(
        ...
        "windows_foundation property value ABI tests must cover default vtable E_NOTIMPL out-slot clearing",
    )
    # The async default-vtable coverage moved to windows_future alongside the
    ...
    require_text_fragments(
        ...
        "windows_foundation foundation default vtable ABI tests must cover E_NOTIMPL out-slot clearing",
    )
    require_text_fragments(
        ...
        "windows_future async default vtable ABI tests must cover E_NOTIMPL out-slot clearing",
    )
    require_text_fragments(
        ...
        "windows_future async derived info thunk cleanup tests must cover derived paths",
    )
    ...
    for marker, label in spawn_sections:
        ...
        require_text_fragments(
            ...
            f"windows_future {label} must preserve WindowsException HRESULTs before generic E_FAIL fallback",
        )
        require_text_before(
            section,
            "catch (error: windows_core.WindowsException)",
            "catch (_: Exception)",
            f"windows_future {label} must catch WindowsException before generic Exception",
        )
        require_text_before(
            section,
            "impl.fail(toFoundationHResult(error.code()))",
            "impl.fail(asyncFailureCode)",
            f"windows_future {label} must record the original WindowsException HRESULT before generic E_FAIL",
        )
    require_text_fragments(
        async_tests.read_text(encoding="utf-8"),
        (
        ...
        "windows_future async spawnAsync tests must cover WindowsException HRESULT preservation",
    )
    require_text_fragments(
        ...
        "windows_foundation memory buffer thunk tests must cover scalar and generic out exception paths",
    )
    ...
    activation_section = section_between(
        ...
        "windows_foundation IGetActivationFactory_Impl",
    )
    activation_body = function_body(activation_section, "__winrtThunk_GetActivationFactory")
    if activation_body is None:
        fail("windows_foundation IGetActivationFactory_Impl must define __winrtThunk_GetActivationFactory")
    dispatch = activation_body.find("this.GetActivationFactory(")
    if dispatch < 0:
        fail("windows_foundation IGetActivationFactory thunk must dispatch to the implementation")
    prefix = activation_body[:dispatch]
    if "result__.write(CPointer<Unit>())" not in prefix:
        fail("windows_foundation IGetActivationFactory thunk must clear its out slot before dispatch")
    if (
        "try {" not in prefix
        ...
        fail("windows_foundation IGetActivationFactory thunk must translate thrown exceptions to HRESULT values")


def check_array_materialization_cleanup(workspace: Path) -> None:
    array_text = (workspace / "windows_core" / "src" / "array.cj").read_text(encoding="utf-8")
    native_array_with_len = section_between(
        ...
        "windows_core NativeAbiArrayStorage.withLen",
    )
    for pattern in (
        ...
        if pattern not in native_array_with_len:
            fail("windows_core NativeAbiArrayStorage.withLen must reject CoTaskMemAlloc failure")
    native_array_clear = section_between(
        array_text,
        "public func clear(): Unit",
        "public func rawOrNull(): CPointer<T>",
        "windows_core NativeAbiArrayStorage.clear",
    )
    if "coTaskMemFree(CPointer<Unit>(data))" not in native_array_clear:
        fail("windows_core NativeAbiArrayStorage.clear must free owned CoTaskMem arrays")
    native_into_abi_start = array_text.find("public func intoAbi(): (CPointer<T>, UInt32)")
    if native_into_abi_start < 0:
        fail("missing windows_core AbiArray.intoAbi")
    ...
    if "native.detach()" not in native_into_abi or "closed = true" not in native_into_abi:
        fail("windows_core AbiArray.intoAbi must detach storage exactly once and close the owner")

    array_proxy_text = (workspace / "windows_core" / "src" / "array_proxy.cj").read_text(encoding="utf-8")
    release_slots = section_between(
        array_proxy_text,
        "func releasePublishedObjectSlots",
        "func publishCopyArray",
        "windows_core ArrayProxy object slot cleanup",
    )
    for pattern in (
        ...
        if pattern not in release_slots:
            fail("windows_core ArrayProxy must release every published HSTRING/interface slot kind")
    publish_object_array = section_between(
        array_proxy_text,
        "func publishObjectArray",
        "// ArrayProxy stages an ABI array",
        "windows_core ArrayProxy object array publisher",
    )
    for pattern in (
        ...
        if pattern not in publish_object_array:
            fail("windows_core ArrayProxy object arrays must clean partial HSTRING/interface publication failures")

    winrt_generic_text = (workspace / "windows_core" / "src" / "winrt_generic.cj").read_text(encoding="utf-8")
    take_slot = section_between(
        winrt_generic_text,
        "static func takeWinrtHandleSlot",
        "static func releaseWinrtHandle",
        "windows_core HandleWinrtType.takeWinrtHandleSlot",
    )
    if "slot.write(CPointer<Unit>())" not in take_slot:
        fail("windows_core HandleWinrtType.takeWinrtHandleSlot must clear transferred raw slots")
    release_owned_slots = section_between(
        winrt_generic_text,
        "func releaseOwnedWinrtHandleSlots",
        "public open class WinrtOutputBridge",
        "windows_core releaseOwnedWinrtHandleSlots",
    )
    for pattern in (
    ...
    for pattern in (
        ...
        if pattern not in handle_take_array_out:
            fail("windows_core HandleWinrtOutputBridge.takeArrayOut must clean partial materialization failures")
    for label, bridge_marker in (
        ...
        bridge_section = section_between(
            ...
            bridge_marker,
            "public override func storeOut(",
            f"windows_core {label}.takeArrayOut",
        )
        for pattern in (
            ...
            if pattern not in bridge_section:
                fail(f"windows_core {label}.takeArrayOut must free CoTaskMem arrays on all paths")
    ...
    if "let raw = value.intoAbi()" in foundation_runtime_text:
        fail("windows_foundation Foundation interface-return thunks must retain returned wrappers instead of consuming intoAbi()")
    descriptor_codegen_text = (workspace / "windows_implement" / "src" / "descriptor_codegen.cj").read_text(encoding="utf-8")
    if "value.intoAbi()" in descriptor_codegen_text:
        fail("windows_implement descriptor return thunks must retain returned wrappers instead of consuming intoAbi()")
    if "windows_core.winrtStoreGenericOut<" not in descriptor_codegen_text:
        fail("windows_implement descriptor return thunks must publish WinRT outputs through winrtStoreGenericOut")
    if "${descriptor.wrapperTypeName}_${method.name}Thunk" not in descriptor_codegen_text:
        fail("windows_implement descriptor thunks must use Cangjie-safe wrapper identifiers")
    if " <: windows_core.Interface<" not in descriptor_codegen_text:
        fail("windows_implement descriptor wrappers must emit Interface<T> marker extensions")
    string_projector = section_between(
        ...
        "windows_foundation PropertyValue string input projector",
    )
    for pattern in (
        "try {",
        "catch (error: Exception)",
        ...
        if pattern not in string_projector:
            fail("windows_foundation PropertyValue string array projection must release partially copied HSTRINGs")
    string_raw_release = section_between(
        foundation_runtime_text,
        "func releasePropertyValueStringRawSlots",
        "func releasePropertyValueInspectableRaw",
        "windows_foundation PropertyValue string raw slot cleanup",
    )
    for pattern in (
        "releaseSystemHStringHandle(raw)",
        "slot.write(CPointer<Unit>())",
        ...
        if pattern not in string_raw_release:
            fail("windows_foundation PropertyValue string raw slot cleanup must release and clear slots")
    string_take_array = section_between(
        foundation_runtime_text,
        "func takePropertyValueStringArray",
        "func releasePropertyValueInspectableRawSlots",
        "windows_foundation PropertyValue string array materializer",
    )
    for pattern in (
        ...
        if pattern not in string_take_array:
            fail("windows_foundation PropertyValue string array materializer must clean partial materialization failures")
    inspectable_raw_release = section_between(
        foundation_runtime_text,
        "func releasePropertyValueInspectableRawSlots",
        "func closePropertyValueInspectableArray",
        "windows_foundation PropertyValue inspectable raw slot cleanup",
    )
    for pattern in (
        ...
        if pattern not in inspectable_raw_release:
            fail("windows_foundation PropertyValue inspectable raw slot cleanup must release and clear slots")
    inspectable_take_array = section_between(
        foundation_runtime_text,
        "func takePropertyValueInspectableArray",
        "func takePropertyValueGuidArray",
        "windows_foundation PropertyValue inspectable array materializer",
    )
    for pattern in (
        ...
        if pattern not in inspectable_take_array:
            fail("windows_foundation PropertyValue inspectable array materializer must clean partial materialization failures")
...
def check_no_shell_scripts(workspace: Path) -> None:
    ...
    for path in sorted(workspace.rglob("*")):
        ...
        if path.suffix.lower() not in FORBIDDEN_SCRIPT_EXTENSIONS:
            continue
        relative = windows_sys_manifest.rel_posix(path, workspace)
...
def check_generated_common_package(workspace: Path) -> None:
    generated = workspace / "windows_sys"
    check_path_exists(generated / "cjpm.toml", "windows_sys cjpm.toml")
    check_path_exists(generated / "codegen-manifest.json", "windows_sys codegen-manifest.json")
    check_path_exists(generated / "src" / "mod.cj", "windows_sys mod.cj")
    try:
        manifest = windows_sys_manifest.validate_manifest_hashes(
            generated,
            expected_header=GENERATED_MANIFEST_HEADER,
        )
    except windows_sys_manifest.ManifestError as error:
        fail(str(error))
    manifest_files = set(windows_sys_manifest.manifest_files(manifest))
    ...
    if package.get("name") != "windows_sys":
        fail(f"windows_sys package name must be windows_sys, got {package.get('name')!r}")
    if manifest.get("package_name") != "windows_sys":
        fail(f"windows_sys manifest package name must be windows_sys, got {manifest.get('package_name')!r}")
    ...
    expected_support_deps = PACKAGE_DEPENDENCIES["windows_sys"]
    if declared_deps != expected_support_deps:
        ...
        fail(f"windows_sys cjpm.toml dependencies mismatch; missing={missing}, extra={extra}")
    if "link-option" in (generated / "cjpm.toml").read_text(encoding="utf-8"):
        fail("windows_sys must not emit static link options; native calls go through windows_libloading")

    check_generated_common_impl_invariants(generated)
    check_windows_sys_native_checked_wrappers(workspace)


def check_generated_common_impl_invariants(generated: Path) -> None:
    impl_dir = generated / "src" / "impl"
    check_path_exists(impl_dir, "windows_sys impl symbol directory")
    impl_files = sorted(impl_dir.glob("symbols_*.cj"))
    if not impl_files:
        fail("windows_sys impl symbol directory must contain symbols_*.cj chunks")
    impl_text = "\n".join(path.read_text(encoding="utf-8") for path in impl_files)
    if "func Start(callback: CPointer<Unit>): Int32" not in impl_text:
        fail("windows_sys IApplicationStatics.Start must accept a raw WinRT delegate COM pointer")
    if "public type Microsoft_UI_Xaml_ApplicationInitializationCallback = CFunc" in impl_text:
        fail("windows_sys must not render WinRT delegate projections as CFunc aliases")
    for _, header, body in iter_cj_class_blocks(impl_text):
        ...
        if "<: Resource" in header or "& Resource" in header:
            ...
            for fragment in required_fragments:
                if fragment not in body:
                    fail(
                        "windows_sys raw handle Resource wrappers must preserve "
        ...
        match = re.search(r"\bclass\s+([A-Za-z_][A-Za-z0-9_]*)", header)
        class_name = match.group(1) if match is not None else header.strip()
        fail(f"windows_sys raw handle classes must be owning Resource wrappers: {class_name}")
    ...
    if "public open class Microsoft_UI_Xaml_ApplicationInitializationCallback <: windows_interface.InterfaceWrapperBase & windows_interface.ComInterface" not in impl_text:
        fail("windows_sys WinRT delegate ApplicationInitializationCallback must project as a ComInterface delegate wrapper")
    if "private class OwnedMicrosoft_UI_Xaml_ApplicationInitializationCallback <: Microsoft_UI_Xaml_ApplicationInitializationCallback" not in impl_text:
        fail("windows_sys WinRT delegate ApplicationInitializationCallback must provide an owned-handle variant for ABI cleanup")
    if "winrtBorrowGenericIn<Microsoft_UI_Xaml_ApplicationInitializationCallback>(callback)" not in impl_text:
        fail("windows_sys Application.Start must borrow the delegate handle through winrtBorrowGenericIn")
    if "Microsoft_UI_Xaml_IApplication.signature()" in impl_text:
        fail("windows_sys runtime class signatures must not call ambiguous interface signature() members")
    if "let _ = hr__.ok()" in impl_text:
        fail("windows_sys runtime class factory wrappers must throw on failed HRESULT values")


def check_windows_sys_native_checked_wrappers(workspace: Path) -> None:
    native_root = workspace / "windows_sys" / "src"
    check_path_exists(native_root, "windows_sys generated native helper root")
    checked_count = 0
    ...
    metadata_payloads = requested_win32_metadata_payloads(workspace)
    check_required_set_last_error_bool_wrappers(metadata_payloads, native_helpers_by_namespace)
    ...
    if inspectable_checked_count == 0:
        fail("windows_sys generated native helpers must include checked Win32 wrappers")
...
def requested_win32_metadata(workspace: Path) -> tuple[Path | None, list[str]]:
    metadata_root = workspace / ".generated" / "winmd-json"
    manifest_root = workspace / "windows_sys"
    if not metadata_root.exists() or not (manifest_root / "codegen-manifest.json").exists():
        return None, []
    try:
        manifest = windows_sys_manifest.load_manifest(
            manifest_root,
            expected_header=GENERATED_MANIFEST_HEADER,
        )
    except windows_sys_manifest.ManifestError as error:
        fail(str(error))
    requested_features = windows_sys_manifest.requested_features(manifest, prefix="Windows.Win32.")
    return metadata_root, requested_features


def requested_win32_metadata_payloads(workspace: Path) -> Win32MetadataPayloads:
    metadata_root, requested_features = requested_win32_metadata(workspace)
    ...
    return payloads, requested_features


def iter_requested_win32_type_defs(metadata_payloads: Win32MetadataPayloads) -> Iterator[Win32TypeDef]:
    payloads, requested_features = metadata_payloads
    for payload in payloads:
        for type_def in payload.get("types", []):
            ...
            if not isinstance(namespace, str) or not namespace_matches_any_feature(namespace, requested_features):
                continue
            yield namespace, type_def


def required_when(predicate: Callable[[dict], bool]) -> NativeWrapperRuleMatcher:
    def match(method: dict) -> object | None:
        return True if predicate(method) else None

    return match


def required_nonempty(match_required: Callable[[dict], Sequence[object]]) -> NativeWrapperRuleMatcher:
    def match(method: dict) -> object | None:
        result = match_required(method)
        return result if result else None

    return match


def check_required_native_wrapper_rule(
    metadata_payloads: Win32MetadataPayloads,
    native_helpers_by_namespace: NativeHelpersByNamespace,
    checked_wrappers_by_namespace: CheckedWrappersByNamespace,
    match_required: NativeWrapperRuleMatcher,
    find_issues: NativeWrapperRuleIssueFinder,
    missing_message: str,
    invalid_message: str,
    uninspectable_message: str = "{namespace}.{name}Checked must be inspectable",
) -> None:
    missing: list[str] = []
    invalid: list[str] = []
    for namespace, type_def in iter_requested_win32_type_defs(metadata_payloads):
        ...
        for method in type_def.get("Methods", []):
            ...
            if wrapper_info is None:
                invalid.append(uninspectable_message.format(namespace=namespace, name=name))
                continue
            wrapper, relative, line = wrapper_info
            issues = find_issues(wrapper, name, match)
            for issue in issues:
                invalid.append(f"{relative}:{line} {name}Checked {issue}")
    if missing:
        fail(f"{missing_message}: {missing[:20]}")
    if invalid:
        fail(f"{invalid_message}: {invalid[:20]}")
    ...
    for namespace, type_def in iter_requested_win32_type_defs(metadata_payloads):
        ...
        for method in type_def.get("Methods", []):
            ...
            if name not in checked_funcs:
                missing.append(f"{namespace}.{name}")
    if missing:
        fail(f"windows_sys missing SetLastError Bool ABI checked wrapper(s): {missing[:20]}")
...
def check_required_multiple_success_bool_status_returns(
    ...
    if invalid:
        fail(f"windows_sys invalid multi-success BOOL native helper return(s): {invalid[:20]}")
...
def check_required_set_last_error_bool_false_success_wrappers(
    ...
    check_required_native_wrapper_rule(
        metadata_payloads,
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        method_set_last_error_bool_false_success_statuses,
        set_last_error_bool_false_success_wrapper_shape_issues,
        "windows_sys missing SetLastError Bool false-success checked wrapper(s)",
        "windows_sys invalid SetLastError Bool false-success checked wrapper(s)",
    )
...
def check_required_set_last_error_bool_status_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        method_set_last_error_bool_statuses,
        set_last_error_bool_status_wrapper_shape_issues,
        "windows_sys missing SetLastError Bool status checked wrapper(s)",
        "windows_sys invalid SetLastError Bool status checked wrapper(s)",
    )
...
def check_required_set_last_error_handle_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        method_set_last_error_null_handle_return_type,
        set_last_error_handle_wrapper_shape_issues,
        "windows_sys missing SetLastError null-handle ABI checked wrapper(s)",
        "windows_sys invalid SetLastError null-handle ABI checked wrapper(s)",
    )
...
def check_required_set_last_error_invalid_handle_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        method_set_last_error_invalid_handle_return_type,
        set_last_error_invalid_handle_wrapper_shape_issues,
        "windows_sys missing SetLastError invalid-handle ABI checked wrapper(s)",
        "windows_sys invalid SetLastError invalid-handle ABI checked wrapper(s)",
    )
...
def check_required_set_last_error_null_success_handle_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        method_set_last_error_null_success_handle_return_type,
        set_last_error_null_success_handle_wrapper_shape_issues,
        "windows_sys missing SetLastError null-success handle ABI checked wrapper(s)",
        "windows_sys invalid SetLastError null-success handle ABI checked wrapper(s)",
    )
...
def check_required_set_last_error_wait_event_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_set_last_error_wait_event_wrapper),
        lambda wrapper, name, _match: set_last_error_wait_event_wrapper_shape_issues(wrapper, name),
        "windows_sys missing SetLastError WAIT_EVENT checked wrapper(s)",
        "windows_sys invalid SetLastError WAIT_EVENT checked wrapper(s)",
    )
...
def check_required_set_last_error_null_pointer_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        method_set_last_error_null_pointer_return_type,
        set_last_error_null_pointer_wrapper_shape_issues,
        "windows_sys missing SetLastError null-pointer checked wrapper(s)",
        "windows_sys invalid SetLastError null-pointer checked wrapper(s)",
    )
...
def check_required_set_last_error_tls_value_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_set_last_error_tls_value_wrapper),
        lambda wrapper, name, _match: set_last_error_tls_value_wrapper_shape_issues(wrapper, name),
        "windows_sys missing SetLastError TLS value checked wrapper(s)",
        "windows_sys invalid SetLastError TLS value checked wrapper(s)",
    )
...
def check_required_set_last_error_uint32_minus_one_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_set_last_error_uint32_minus_one_wrapper),
        lambda wrapper, name, _match: set_last_error_uint32_minus_one_wrapper_shape_issues(wrapper, name),
        "windows_sys missing SetLastError UInt32 -1 checked wrapper(s)",
        "windows_sys invalid SetLastError UInt32 -1 checked wrapper(s)",
    )
...
def check_required_set_last_error_uint32_zero_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_set_last_error_uint32_zero_wrapper),
        lambda wrapper, name, _match: set_last_error_uint32_zero_wrapper_shape_issues(wrapper, name),
        "windows_sys missing SetLastError UInt32 zero checked wrapper(s)",
        "windows_sys invalid SetLastError UInt32 zero checked wrapper(s)",
    )
...
def check_required_set_last_error_uint32_zero_last_error_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_set_last_error_uint32_zero_last_error_wrapper),
        lambda wrapper, name, _match: set_last_error_uint32_zero_last_error_wrapper_shape_issues(wrapper, name),
        "windows_sys missing SetLastError UInt32 zero-last-error checked wrapper(s)",
        "windows_sys invalid SetLastError UInt32 zero-last-error checked wrapper(s)",
    )
...
def check_required_set_last_error_uintnative_zero_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_set_last_error_uintnative_zero_wrapper),
        lambda wrapper, name, _match: set_last_error_uintnative_zero_wrapper_shape_issues(wrapper, name),
        "windows_sys missing SetLastError UIntNative zero checked wrapper(s)",
        "windows_sys invalid SetLastError UIntNative zero checked wrapper(s)",
    )
...
def check_required_set_last_error_intnative_zero_last_error_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_set_last_error_intnative_zero_last_error_wrapper),
        lambda wrapper, name, _match: set_last_error_intnative_zero_last_error_wrapper_shape_issues(wrapper, name),
        "windows_sys missing SetLastError IntNative zero-last-error checked wrapper(s)",
        "windows_sys invalid SetLastError IntNative zero-last-error checked wrapper(s)",
    )
...
def check_required_set_last_error_int32_max_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_set_last_error_int32_max_wrapper),
        lambda wrapper, name, _match: set_last_error_int32_max_wrapper_shape_issues(wrapper, name),
        "windows_sys missing SetLastError Int32 max checked wrapper(s)",
        "windows_sys invalid SetLastError Int32 max checked wrapper(s)",
    )
...
def check_required_set_last_error_int32_minus_one_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_set_last_error_int32_minus_one_wrapper),
        lambda wrapper, name, _match: set_last_error_int32_minus_one_wrapper_shape_issues(wrapper, name),
        "windows_sys missing SetLastError Int32 -1 checked wrapper(s)",
        "windows_sys invalid SetLastError Int32 -1 checked wrapper(s)",
    )
...
def check_required_winsock_invalid_socket_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_winsock_invalid_socket_wrapper),
        lambda wrapper, name, _match: winsock_invalid_socket_wrapper_shape_issues(wrapper, name),
        "windows_sys missing WinSock INVALID_SOCKET checked wrapper(s)",
        "windows_sys invalid WinSock INVALID_SOCKET checked wrapper(s)",
    )
...
def check_required_winsock_bool_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_winsock_bool_wrapper),
        lambda wrapper, name, _match: winsock_bool_wrapper_shape_issues(wrapper, name),
        "windows_sys missing WinSock BOOL checked wrapper(s)",
        "windows_sys invalid WinSock BOOL checked wrapper(s)",
    )
...
def check_required_winsock_wait_event_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_winsock_wait_event_wrapper),
        lambda wrapper, name, _match: winsock_wait_event_wrapper_shape_issues(wrapper, name),
        "windows_sys missing WinSock WAIT_EVENT checked wrapper(s)",
        "windows_sys invalid WinSock WAIT_EVENT checked wrapper(s)",
    )
...
def check_required_winsock_invalid_event_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_winsock_invalid_event_wrapper),
        lambda wrapper, name, _match: winsock_invalid_event_wrapper_shape_issues(wrapper, name),
        "windows_sys missing WinSock WSA_INVALID_EVENT checked wrapper(s)",
        "windows_sys invalid WinSock WSA_INVALID_EVENT checked wrapper(s)",
    )
...
def check_required_winsock_null_handle_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_winsock_null_handle_wrapper),
        lambda wrapper, name, _match: winsock_null_wrapper_shape_issues(wrapper, name, "CPointer<Unit>"),
        "windows_sys missing WinSock null-handle checked wrapper(s)",
        "windows_sys invalid WinSock null-handle checked wrapper(s)",
    )
...
def check_required_winsock_null_pointer_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        method_winsock_null_pointer_return_type,
        winsock_null_wrapper_shape_issues,
        "windows_sys missing WinSock null-pointer checked wrapper(s)",
        "windows_sys invalid WinSock null-pointer checked wrapper(s)",
    )
...
def check_required_winsock_socket_error_int32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_winsock_socket_error_int32_wrapper),
        lambda wrapper, name, _match: winsock_socket_error_int32_wrapper_shape_issues(
            wrapper,
            name,
            name not in WINSOCK_SOCKET_ERROR_UNIT_INT32_METHODS,
        ),
        "windows_sys missing WinSock SOCKET_ERROR Int32 checked wrapper(s)",
        "windows_sys invalid WinSock SOCKET_ERROR Int32 checked wrapper(s)",
    )
...
def check_required_mswinsock_get_last_error_socket_error_int32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_mswinsock_get_last_error_socket_error_int32_wrapper),
        lambda wrapper, name, _match: mswinsock_get_last_error_socket_error_int32_wrapper_shape_issues(
            wrapper,
            name,
            name not in MSWSOCK_GET_LAST_ERROR_SOCKET_ERROR_UNIT_INT32_METHODS,
        ),
        "windows_sys missing MSWSOCK GetLastError SOCKET_ERROR Int32 checked wrapper(s)",
        "windows_sys invalid MSWSOCK GetLastError SOCKET_ERROR Int32 checked wrapper(s)",
    )
...
def check_required_winsock_direct_error_int32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_winsock_direct_error_int32_wrapper),
        lambda wrapper, name, _match: winsock_direct_error_int32_wrapper_shape_issues(wrapper, name),
        "windows_sys missing WinSock direct-error Int32 checked wrapper(s)",
        "windows_sys invalid WinSock direct-error Int32 checked wrapper(s)",
    )
...
def check_required_winsock_pending_status_int32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        method_winsock_pending_status_int32,
        winsock_pending_status_int32_wrapper_shape_issues,
        "windows_sys missing WinSock pending-status Int32 checked wrapper(s)",
        "windows_sys invalid WinSock pending-status Int32 checked wrapper(s)",
    )
...
def check_required_winsock_lperrno_int32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        method_winsock_lperrno_int32_status,
        winsock_lperrno_int32_wrapper_shape_issues,
        "windows_sys missing WinSock lpErrno Int32 checked wrapper(s)",
        "windows_sys invalid WinSock lpErrno Int32 checked wrapper(s)",
    )
...
def check_required_wait_failed_uint32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_wait_failed_uint32_wrapper),
        lambda wrapper, name, _match: wait_failed_uint32_wrapper_shape_issues(wrapper, name),
        "windows_sys missing WAIT_FAILED UInt32 checked wrapper(s)",
        "windows_sys invalid WAIT_FAILED UInt32 checked wrapper(s)",
    )


def direct_win32_error_code_int32_rule_match(method: dict) -> tuple[str, object] | None:
    success_statuses = method_direct_win32_error_code_int32_success_statuses(method)
    if success_statuses is not None:
        return "status", success_statuses
    if method_needs_direct_win32_error_code_int32_wrapper(method):
        return "direct", True
    return None


def direct_win32_error_code_int32_rule_issues(wrapper: str, name: str, match: object) -> list[str]:
    mode, payload = match
    if mode == "status":
        return direct_win32_error_code_int32_status_wrapper_shape_issues(wrapper, name, payload)
    return direct_win32_error_code_int32_wrapper_shape_issues(wrapper, name)
...
def check_required_direct_win32_error_code_int32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        direct_win32_error_code_int32_rule_match,
        direct_win32_error_code_int32_rule_issues,
        "windows_sys missing direct Win32 error-code Int32 checked wrapper(s)",
        "windows_sys invalid direct Win32 error-code Int32 checked wrapper(s)",
    )


def direct_win32_error_code_uint32_rule_match(method: dict) -> tuple[str, object] | None:
...
def direct_win32_error_code_uint32_rule_issues(wrapper: str, name: str, match: object) -> list[str]:
    mode, payload = match
    if mode == "status":
        return direct_win32_error_code_uint32_status_wrapper_shape_issues(wrapper, name, payload)
    return direct_win32_error_code_uint32_wrapper_shape_issues(wrapper, name)
...
def check_required_direct_win32_error_code_uint32_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        direct_win32_error_code_uint32_rule_match,
        direct_win32_error_code_uint32_rule_issues,
        "windows_sys missing direct Win32 error-code UInt32 checked wrapper(s)",
        "windows_sys invalid direct Win32 error-code UInt32 checked wrapper(s)",
    )
...
def check_required_uint32_less_than_or_equal_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        method_uint32_less_than_or_equal_failure_threshold,
        uint32_less_than_or_equal_wrapper_shape_issues,
        "windows_sys missing UInt32 <= threshold checked wrapper(s)",
        "windows_sys invalid UInt32 <= threshold checked wrapper(s)",
    )
...
def check_required_hresult_com_out_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_nonempty(lambda method: method_hresult_com_out_slot_names(method, type_attributes)),
        hresult_com_out_wrapper_shape_issues,
        "windows_sys missing HRESULT COM out ABI checked wrapper(s)",
        "windows_sys invalid HRESULT COM out ABI checked wrapper(s)",
        uninspectable_message="{namespace}.{name}Checked must return windows_core.Result<Unit>",
    )
...
def check_required_hresult_owned_string_out_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_nonempty(method_hresult_owned_string_out_slots),
        hresult_owned_string_out_wrapper_shape_issues,
        "windows_sys missing HRESULT owned string out ABI checked wrapper(s)",
        "windows_sys invalid HRESULT owned string out ABI checked wrapper(s)",
        uninspectable_message="{namespace}.{name}Checked must return windows_core.Result<Unit>",
    )
...
def check_required_hresult_owned_pointer_out_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_nonempty(method_hresult_owned_pointer_out_slots),
        hresult_owned_pointer_out_wrapper_shape_issues,
        "windows_sys missing HRESULT owned pointer out ABI checked wrapper(s)",
        "windows_sys invalid HRESULT owned pointer out ABI checked wrapper(s)",
        uninspectable_message="{namespace}.{name}Checked must return windows_core.Result<Unit>",
    )
...
def check_required_hresult_borrowed_optional_out_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_nonempty(method_hresult_borrowed_optional_out_slots),
        hresult_borrowed_optional_out_wrapper_shape_issues,
        "windows_sys missing HRESULT borrowed optional out ABI checked wrapper(s)",
        "windows_sys invalid HRESULT borrowed optional out ABI checked wrapper(s)",
        uninspectable_message="{namespace}.{name}Checked must return windows_core.Result<Unit>",
    )
...
def check_required_hresult_optional_scalar_out_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_nonempty(method_hresult_optional_scalar_out_slots),
        hresult_optional_scalar_out_wrapper_shape_issues,
        "windows_sys missing HRESULT optional scalar out ABI checked wrapper(s)",
        "windows_sys invalid HRESULT optional scalar out ABI checked wrapper(s)",
        uninspectable_message="{namespace}.{name}Checked must return windows_core.Result<Unit>",
    )
...
def check_required_multiple_success_hresult_wrappers(
    ...
    check_required_native_wrapper_rule(
        ...
        native_helpers_by_namespace,
        checked_wrappers_by_namespace,
        required_when(method_needs_multiple_success_hresult_wrapper),
        lambda wrapper, name, _match: multiple_success_hresult_wrapper_shape_issues(wrapper, name),
        "windows_sys missing multiple-success HRESULT checked wrapper(s)",
        "windows_sys invalid multiple-success HRESULT checked wrapper(s)",
        uninspectable_message="{namespace}.{name}Checked must preserve the HRESULT success value",
    )
...
def check_package_boundaries(workspace: Path) -> None:
    ...
    macro_tests = workspace / "windows_interface" / "tests"
    check_path_exists(macro_tests, "windows_interface macro tests")
    macro_fixtures = sorted((macro_tests / "macros").glob("*.cj"))
    if not macro_fixtures:
        fail("windows_interface macro tests must include at least one .cj fixture")
    ...
    if macro_import_packages != WINDOWS_INTERFACE_MACRO_TEST_LINK_CLOSURE:
        ...
        fail(f"windows_interface macro test link closure mismatch; missing={missing}, extra={extra}")

    interface_native = workspace / "windows_interface" / "src" / "native.cj"
    if interface_native.exists():
        fail("implementation native helpers must not live in windows_interface/src/native.cj")
    interface_wrapper_text = (workspace / "windows_interface" / "src" / "interface_wrapper.cj").read_text(encoding="utf-8")
    iinspectable_vtbl_section = section_between(
    ...
    if iinspectable_vtbl_section.count("if (valuePtr.isNull())") < 2:
        fail("windows_interface default IInspectableVtbl must reject null runtime-name and trust-level out pointers")

    implement_native = workspace / "windows_implement" / "src" / "native.cj"
    check_path_exists(implement_native, "windows_implement native helper")
    ...
    if "CoCreateFreeThreadedMarshaler" not in implement_native_text:
        fail("windows_implement native helper must own CoCreateFreeThreadedMarshaler")
    ...
    if ".lock()" in hstring_ref_count_text or ".unlock()" in hstring_ref_count_text:
        fail("windows_strings HSTRING refcount state must use synchronized blocks")
    bstr_text = (workspace / "windows_strings" / "src" / "bstr.cj").read_text(encoding="utf-8")
    if "unsafeFromRaw" in bstr_text or "unsafeAttach" in bstr_text:
        fail("windows_strings BSTR raw constructors must describe ownership instead of using unsafe* names")
    for source in (workspace / "windows_strings" / "src").rglob("*.cj"):
    ...
    strings_native = workspace / "windows_strings" / "src" / "native.cj"
    check_path_exists(strings_native, "windows_strings native helper")
    for source in (workspace / "windows_strings" / "src").rglob("*.cj"):
        text = source.read_text(encoding="utf-8")
        if source != strings_native and "windows_libloading" in text:
            fail(f"{source.relative_to(workspace)} must use windows_strings native helpers instead of loading symbols directly")

    threading_lib = workspace / "windows_threading" / "src" / "lib.cj"
    threading_lib_text = threading_lib.read_text(encoding="utf-8")
    if ".lock()" in threading_lib_text or ".unlock()" in threading_lib_text:
        fail("windows_threading state guards must use synchronized blocks")

    services_lib = workspace / "windows_services" / "src" / "lib.cj"
    services_lib_text = services_lib.read_text(encoding="utf-8")
    if ".lock()" in services_lib_text or ".unlock()" in services_lib_text:
        fail("windows_services state guards must use synchronized blocks")

    runtime_async = workspace / "windows_future" / "src" / "async_helpers.cj"
    runtime_async_text = runtime_async.read_text(encoding="utf-8")
    if ".lock()" in runtime_async_text or ".unlock()" in runtime_async_text:
        fail("windows_future async state guards must use synchronized blocks")

    core_type_impl = workspace / "windows_core" / "src" / "type_impl.cj"
    ...
    if "fromAbiTakeWinrtHandle(raw: CPointer<Unit>): HString { HString.fromAbiTake(raw) }" in core_type_impl_text:
        fail("windows_core HString WinRT out handles must use system HSTRING ownership")
    if "viewWinrtHandle(raw: CPointer<Unit>): HString { HString.viewOf(raw) }" in core_type_impl_text:
        fail("windows_core HString WinRT input handles must copy borrowed system HSTRINGs")
    if "intoWinrtHandle(value: HString): CPointer<Unit> { value.asRaw() }" in core_type_impl_text:
        fail("windows_core HString WinRT handles must publish system HSTRING copies")
    core_ref_param_text = (workspace / "windows_core" / "src" / "ref_param.cj").read_text(encoding="utf-8")
    if "case hstring: HString" in core_ref_param_text and "hstring.asRaw()" in core_ref_param_text:
        fail("windows_core InParam<HString> must not publish project-managed HSTRING handles")
    winrt_generic_text = (workspace / "windows_core" / "src" / "winrt_generic.cj").read_text(encoding="utf-8")
    if "SystemHStringWinrtInputBorrow" not in winrt_generic_text or "borrowedWinrtInputHandle<T>(value)" not in winrt_generic_text:
        fail("windows_core HString generic input borrows must publish system HSTRING copies")
    hstring_borrow_section = section_between(
        winrt_generic_text,
        "class SystemHStringWinrtInputBorrow",
        "func borrowedWinrtInputHandle",
        "windows_core HString generic input borrow",
    )
    if "raw = value.toSystemHandleCopy()" not in hstring_borrow_section:
        fail("windows_core HString generic input borrow must create a system HSTRING copy")
    if "releaseSystemHStringHandle(handle)" not in hstring_borrow_section:
        fail("windows_core HString generic input borrow must release system HSTRING copies without allocating")
    if "HString.fromSystemHandleTake(raw).close()" in hstring_borrow_section:
        fail("windows_core HString generic input borrow release must not allocate while closing")
    owned_winrt_handle_section = section_between(
        winrt_generic_text,
        "func ownedWinrtOutHandle",
        "public open class WinrtOutputBridge",
        "windows_core HString generic output handle",
    )
    if "case hstring: HString" not in owned_winrt_handle_section or "hstring.toSystemHandleCopy()" not in owned_winrt_handle_section:
        fail("windows_core HString generic output handles must publish system HSTRING copies")
    if "hstring.asRaw()" in owned_winrt_handle_section or "hstring.clone().intoRaw()" in owned_winrt_handle_section:
        fail("windows_core HString generic output handles must not publish project-managed HSTRING storage")
    array_proxy_text = (workspace / "windows_core" / "src" / "array_proxy.cj").read_text(encoding="utf-8")
    if "value.intoRaw()" in array_proxy_text or "value.clone().intoRaw()" in array_proxy_text:
        fail("windows_core ArrayProxy must publish HString elements as system HSTRING copies")
    array_proxy_release_section = section_between(
        array_proxy_text,
        "func releasePublishedObjectSlots",
        "func publishCopyArray",
        "windows_core ArrayProxy object cleanup",
    )
    if "releaseSystemHStringHandle(raw)" not in array_proxy_release_section:
        fail("windows_core ArrayProxy must release system HSTRING slots without allocating")
    if "HString.fromSystemHandleTake(raw).close()" in array_proxy_release_section:
        fail("windows_core ArrayProxy cleanup must not allocate while releasing system HSTRING slots")
    interface_impl_surface_text = (workspace / "windows_implement" / "src" / "interface_impl_surface.cj").read_text(encoding="utf-8")
    runtime_name_section = section_between(
        ...
        "windows_implement GetRuntimeClassNameBase",
    )
    if "intoRaw" in runtime_name_section:
        fail("windows_implement GetRuntimeClassName must publish system HSTRING copies")
    if "toSystemHandleCopy()" not in runtime_name_section:
        fail("windows_implement GetRuntimeClassName must publish system HSTRING copies")
    implement_surface_text = interface_impl_surface_text
    descriptor_schema_text = (workspace / "windows_implement" / "src" / "descriptor_schema.cj").read_text(encoding="utf-8")
    if "public let vtableAncestorIids: Array<GUID>" not in descriptor_schema_text:
        fail("windows_implement descriptors must distinguish QI metadata from vtable inheritance metadata")
    if "func resolvedDescriptorVtableAncestorDepth(" not in descriptor_schema_text:
        fail("windows_implement slot assignment must compute effective vtable inheritance depth")
    if "func resolvedDescriptorIsComInterface(" not in descriptor_schema_text:
        fail("windows_implement slot assignment must infer COM inheritance through descriptor chains")
    if "func findDeepestDescendantIndex(" not in descriptor_schema_text or "descriptorContainsVtableAncestor(values, index, parent.iid)" not in descriptor_schema_text:
        fail("windows_implement slot assignment must group inherited slots through full ancestor chains")
    if "func explicitCustomVtblArray(" not in implement_surface_text:
        fail("windows_implement must map explicit custom vtables through resolved schema slots")
    if "func requiredDescriptorIndicesForCustomSlots(" not in implement_surface_text:
        fail("windows_implement explicit custom vtable mapping must require the deepest descriptor per slot")
    if 'getStaticFunction("vtablePtr", parseParameterTypes(""))' not in implement_surface_text:
        fail("windows_implement descriptor-backed vtable reflection must target no-arg wrapper vtablePtr()")
    if 'getStaticFunction("vtablePtr", Array<TypeInfo>(0, { _ => TypeInfo.of<Int64>() }))' in implement_surface_text:
        fail("windows_implement descriptor-backed vtable reflection must not look for obsolete vtablePtr(Int64)")
    if "func requireExplicitSchemaCustomVtbls(" not in implement_surface_text:
        fail("windows_implement schema-only factories must reject custom interfaces without vtables")
    if "func requireCompleteCustomVtbls(" not in implement_surface_text:
        fail("windows_implement explicit schema vtable arrays must reject missing custom slots")
    if "explicitCustomVtblArray(resolvedDescriptors, vtblPtrs)" not in implement_surface_text:
        fail("windows_implement schema vtable arrays must be normalized before runtime attachment")
    if "fallbackCustomVtbl" in implement_surface_text:
        fail("windows_implement must not silently fallback missing custom vtables to IUnknown")
    schema_vtable_test = workspace / "windows_implement" / "src" / "schema_vtable_test.cj"
    check_path_exists(schema_vtable_test, "windows_implement schema vtable guard test")
    if "testSchemaOnlyCustomInterfacesRequireExplicitVtables" not in schema_vtable_test.read_text(encoding="utf-8"):
        fail("windows_implement must test schema-only custom interface rejection")
    if "testDescriptorBackedCustomInterfaceReflectsNoArgWrapperVtbl" not in schema_vtable_test.read_text(encoding="utf-8"):
        fail("windows_implement must test descriptor-backed custom interface vtable reflection")
    if "Array<CPointer<Unit>>(0" not in schema_vtable_test.read_text(encoding="utf-8") or "[CPointer<Unit>()]" not in schema_vtable_test.read_text(encoding="utf-8"):
        fail("windows_implement must test missing explicit schema vtable rejection")
    if "testInheritedCustomInterfacesRequireLeafVtable" not in schema_vtable_test.read_text(encoding="utf-8"):
        fail("windows_implement must test that inherited custom slots require leaf vtables")
    if "testQueryInterfacePrefersExactDescriptorBeforeAncestorMatch" not in schema_vtable_test.read_text(encoding="utf-8"):
        fail("windows_implement must test exact QueryInterface matching before ancestor fallback")
    if "testInheritedCustomInterfacesGroupThroughOmittedIntermediate" not in schema_vtable_test.read_text(encoding="utf-8"):
        fail("windows_implement must test inherited slot grouping when intermediate descriptors are omitted")
    if "testDirectBaseIidSchemasGroupThroughListedIntermediate" not in schema_vtable_test.read_text(encoding="utf-8"):
        fail("windows_implement must test direct-base-IID inheritance slot grouping")
    macro_inherited_fixture = (workspace / "windows_interface" / "tests" / "macros" / "interface_implement_fixture.cj").read_text(encoding="utf-8")
    if "@Implement[IFixtureBase, IFixtureDerived]" not in macro_inherited_fixture:
        fail("windows_interface macro fixtures must cover inherited @Implement vtable slot mapping")
    if "inheritedDerived.DerivedValue" not in macro_inherited_fixture:
        fail("windows_interface inherited macro fixture must dispatch through the derived interface")
    if "@Implement[IFixtureSecondDerived]" not in macro_inherited_fixture:
        fail("windows_interface macro fixtures must cover inherited default thunk dispatch")
    if "SecondBaseValue" not in macro_inherited_fixture or "__winrtThunk_SecondBaseValue" in macro_inherited_fixture:
        fail("windows_interface inherited macro fixture must rely on the generated default base thunk")
    factory_cache_text = (workspace / "windows_core" / "src" / "factory_cache.cj").read_text(encoding="utf-8")
    if "classId.asRaw()" in factory_cache_text:
        fail("windows_core activation factory loading must publish class IDs as system HSTRING copies")
    if "callRoGetActivationFactoryWithClassId" not in factory_cache_text or "callDllGetActivationFactory" not in factory_cache_text:
        fail("windows_core activation factory loading must centralize HSTRING input projection")
    if "winrtBorrowGenericIn<HString>(classId)" not in factory_cache_text:
        fail("windows_core activation factory loading must borrow class IDs as system HSTRING copies")
    if "if (incrementHr.failed())" not in factory_cache_text:
        fail("windows_core activation factory loading must preserve CoIncrementMTAUsage failures")
    if "class FactoryMTAUsageGuard" not in factory_cache_text or "let processFactoryMTAUsage" not in factory_cache_text:
        fail("windows_core activation factory loading must retain CoIncrementMTAUsage as a single process guard")
    if "mtaUsage!: FactoryMTAUsageGuard = processFactoryMTAUsage" not in factory_cache_text:
        fail("windows_core activation factory test hook must allow isolated MTA usage guards")
    if "let incrementHr = mtaUsage.ensure(incrementMTAUsageCall)" not in factory_cache_text:
        fail("windows_core activation factory loading must route CoIncrementMTAUsage through the process guard")
    if "cookie = Some(newCookie)" not in factory_cache_text or "newCookie.isNull()" not in factory_cache_text:
        fail("windows_core activation factory MTA guard must store only successful non-null cookies")
    factory_cache_tests = (workspace / "windows_core" / "src" / "factory_cache_test.cj").read_text(encoding="utf-8")
    if (
        ...
        fail("windows_core activation factory tests must cover single-guard MTA usage fallback")
    if (
    ...
    for pattern in (
        ...
        if pattern in foundation_runtime_text:
            fail("windows_foundation direct HSTRING inputs must use system HSTRING input borrows")
    ...
    common_com_helpers = workspace / "windows_sys" / "src" / "Win32" / "System" / "Com" / "native_helpers.cj"
    if common_com_helpers.exists():
        ...
        for pattern in (
            ...
            if re.search(pattern, common_com_text):
                fail("windows_sys COM native helpers must preserve pointer-to-pointer out slots")

    interface_src = workspace / "windows_interface" / "src"
    ...
    libloading = workspace / "windows_libloading" / "src" / "lib.cj"
    libloading_text = libloading.read_text(encoding="utf-8")
    if "LoadLibraryA" in libloading_text:
        fail("windows_libloading must use LoadLibraryExA with an explicit system DLL search policy")
    if "LOAD_LIBRARY_SEARCH_SYSTEM32" not in libloading_text:
        fail("windows_libloading must use a System32-only search policy for system DLLs")
    if "LOAD_LIBRARY_SEARCH_DEFAULT_DIRS" not in libloading_text or "resolveProcFromSearchPath" not in libloading_text:
        fail("windows_libloading must expose an explicit search-path API for app-local DLL loading")
    if "LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR" not in libloading_text or "searchPathLoadFlags(moduleName)" not in libloading_text:
        fail("windows_libloading explicit search-path loads must preserve absolute DLL directories for dependencies")
    if "ensureSystemModuleNameBytes(moduleNameBytes)" not in libloading_text or "isPathLikeModuleName(moduleName)" not in libloading_text:
        fail("windows_libloading system DLL loading must reject path-like module names")
    load_library_section = section_between(
        libloading_text,
        "public func loadLibrary(",
        "public func resolveProc(",
        "windows_libloading loadLibrary",
    )
    if "loadSystemLibraryUncached(moduleNameBytes)" not in load_library_section:
        fail("windows_libloading loadLibrary must load bare system DLLs from System32")
    load_library_locked_section = section_between(
        libloading_text,
        "func loadLibraryLocked(",
        "func loadLibraryFromSearchPathLocked(",
        "windows_libloading loadLibraryLocked",
    )
    if "loadSystemLibraryUncached(moduleNameBytes)" not in load_library_locked_section:
        fail("windows_libloading resolveProc must load bare system DLLs from System32")
    native_helpers_text = (workspace / "windows_bindgen" / "src" / "native_helpers.cj").read_text(encoding="utf-8")
    if "resolveProc(${spec.dynamicModuleParameterName}" in native_helpers_text:
        fail("windows_bindgen dynamic native helpers must use the explicit search-path resolver")
    check_pinvoke_string_out_cleanup_gate(workspace)
    check_native_hresult_optional_out_cleanup_gate(workspace)
    app_runtime_helpers = workspace / "windows_sys" / "src" / "Native" / "AppRuntime" / "native_helpers.cj"
    check_path_exists(app_runtime_helpers, "windows_sys AppRuntime native helpers")
    app_runtime_text = app_runtime_helpers.read_text(encoding="utf-8")
    if "resolveProc(bootstrapDllPath" in app_runtime_text:
        fail("windows_sys AppRuntime helpers must use the explicit search-path resolver")
    if app_runtime_text.count("resolveProcFromSearchPath(bootstrapDllPath") < 2:
        fail("windows_sys AppRuntime helpers must resolve bootstrap exports via the explicit search-path resolver")
    if "MddBootstrapInitialize2" not in app_runtime_text or "MddBootstrapShutdown" not in app_runtime_text:
        fail("windows_sys AppRuntime helpers must include both bootstrap exports")
    common_com_helpers = workspace / "windows_sys" / "src" / "Win32" / "System" / "Com" / "native_helpers.cj"
    ...
    if "public func CoTaskMemAlloc(cb: UIntNative): CPointer<Unit>" not in common_com_helpers_text:
        fail("windows_sys CoTaskMemAlloc helper must expose SIZE_T byte count as UIntNative")
    if "public func CoTaskMemRealloc(pv: CPointer<Unit>, cb: UIntNative): CPointer<Unit>" not in common_com_helpers_text:
        fail("windows_sys CoTaskMemRealloc helper must expose SIZE_T byte count as UIntNative")

    core_native = workspace / "windows_core" / "src" / "native.cj"
    ...
    for primitive in ("GetProcAddress", "LoadLibraryExA", "FreeLibrary"):
        if f"func {primitive}(" in core_native_text:
            fail(f"windows_core native helpers must not wrap {primitive}; use windows_libloading")
...
def check_active_tools(workspace: Path) -> None:
    ...
    if "writer.fileHashes()" not in generator_main_text or "file_hashes" not in generator_main_text:
        fail("windows_bindgen must generate manifest file_hashes for reproducible windows_sys output")
    ...
    common_codegen_gate = workspace / "scripts" / "check_windows_sys_codegen.py"
    check_path_exists(common_codegen_gate, "windows_sys codegen gate")
    ...
    if (
        ...
        fail("full windows_sys codegen gate must require explicit opt-in before skipping missing WinUI metadata")
    if "remove_expected_generator_binary()" not in common_codegen_gate_text or 'with_suffix(".cjo")' not in common_codegen_gate_text:
        fail("windows_sys codegen gate must clear stale generator binaries before rebuilding")
    ...
    try:
        ...
        fail(f"windows_interface macro check script must be valid Python: {exc}")
    if not python_defines_function(macro_check_tree, "cjv_exec_args") or not macro_fixture_uses_cjv_exec(macro_check_tree):
        fail("windows_interface macro fixtures must execute produced binaries through cjv exec")
    if macro_fixture_directly_executes_binary(macro_check_tree):
        fail("windows_interface macro fixtures must not execute produced .exe files directly")
    if (
    ...
    if demo_root.exists() and "windows_winui3" in ACTIVE_WORKSPACE_MEMBERS:
        ...
        if 'windows_winui3 = { path = "../windows-cj/windows_winui3" }' not in demo_cjpm:
...
def check_propvariant_propsys_smoke(workspace: Path) -> None:
    source = workspace / "windows_propvariant" / "src" / "propvariant_test.cj"
    check_path_exists(source, "windows_propvariant PROPVARIANT tests")
    ...
    text = read_tests_text(workspace / "windows_propvariant" / "src")
    ...
    if missing_file:
        fail(f"windows_propvariant must keep real Propsys PROPVARIANT smoke coverage; missing={missing_file}")

    body = function_body(text, "propsysConsumesCangjiePropVariants")
    if body is None:
        fail("windows_propvariant Propsys smoke test body is missing")
    code_body = mask_cj_strings_and_comments(body)
    ...
    if missing_body:
        fail(f"windows_propvariant Propsys smoke no longer consumes Cangjie PROPVARIANTs; missing={missing_body}")


def check_variant_oleaut_smoke(workspace: Path) -> None:
    source = workspace / "windows_variant" / "src" / "variant_test.cj"
    check_path_exists(source, "windows_variant VARIANT tests")
    ...
    text = read_tests_text(workspace / "windows_variant" / "src")
    required_file_markers = (
    ...
    missing_file = [marker for marker in required_file_markers if marker not in text]
    if missing_file:
        fail(f"windows_variant must keep real OleAut32 VARIANT smoke coverage; missing={missing_file}")

    helper_body = function_body(text, "variantSmokeChangeType")
    if helper_body is None:
        fail("windows_variant VariantChangeType smoke helper is missing")
    ...
    if missing_helper:
        fail(f"windows_variant VariantChangeType helper no longer owns output cleanup; missing={missing_helper}")

    body = function_body(text, "oleautVariantChangeTypeConsumesCangjieVariants")
    if body is None:
        fail("windows_variant OleAut32 smoke test body is missing")
    code_body = mask_cj_strings_and_comments(body)
    required_body_markers = (
    ...
    missing_body = [marker for marker in required_body_markers if marker not in code_body]
    if missing_body:
        fail(f"windows_variant OleAut32 smoke no longer consumes Cangjie VARIANTs; missing={missing_body}")


def check_safearray_oleaut_smoke(workspace: Path) -> None:
    source = workspace / "windows_safearray" / "src" / "safearray_test.cj"
    check_path_exists(source, "windows_safearray SAFEARRAY tests")
    ...
    text = read_tests_text(workspace / "windows_safearray" / "src")
    required_file_markers = (
    ...
    missing_file = [marker for marker in required_file_markers if marker not in text]
    if missing_file:
        fail(f"windows_safearray must keep real OleAut32 SAFEARRAY smoke coverage; missing={missing_file}")

    vector_body = function_body(text, "safeArraySmokeVectorFromBstr")
    if vector_body is None:
        fail("windows_safearray VectorFromBstr smoke helper is missing")
    ...
    if missing_vector:
        fail(f"windows_safearray VectorFromBstr helper no longer owns output cleanup; missing={missing_vector}")

    bstr_body = function_body(text, "safeArraySmokeBstrFromVector")
    if bstr_body is None:
        fail("windows_safearray BstrFromVector smoke helper is missing")
    ...
    if missing_bstr:
        fail(f"windows_safearray BstrFromVector helper no longer consumes SAFEARRAY and owns BSTR output; missing={missing_bstr}")

    body = function_body(text, "oleautBstrVectorRoundTripsThroughSafeArray")
    if body is None:
        fail("windows_safearray OleAut32 smoke test body is missing")
    code_body = mask_cj_strings_and_comments(body)
    required_body_markers = (
    ...
    missing_body = [marker for marker in required_body_markers if marker not in code_body]
    if missing_body:
        fail(f"windows_safearray OleAut32 smoke no longer roundtrips through SAFEARRAY; missing={missing_body}")
...
def main() -> None:
    ...
    print("OK: generated windows_sys package is present and marked DO NOT EDIT")
    ...
    print("OK: windows_strings finalizer release paths are lock-free")
