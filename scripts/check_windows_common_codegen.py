...
hashes (against the checked-in integrity record), and the windows-common build
entry. Full mode additionally rebuilds the generator and regenerates
windows-common into a temporary directory for a non-destructive manifest/file
...
reader built into windows-bindgen; the gate never consumes or produces a
...
# windows-bindgen. The gate emits transient per-namespace JSON into a temporary
# directory only when it needs type-level metadata; nothing is ever persisted.
WINMD_NATIVE_HELPER = "cjpm run -m windows-bindgen -- <raw-winmd-root> --feature <namespace> --out <dir>"
...
def check_winmd_source_hashes() -> None:
    ...
    if mismatches:
        fail(
            ...
            "record with --update-source-hashes if the change is intentional (then regenerate windows-common)."
        )
    print(
...
def print_winui_metadata_guidance() -> None:
    ...
    print(
        ...
        f"Regenerate windows-common from raw metadata with: {helper}."
    )
...
def emit_winmd_json_command(winmd_inputs: Sequence[Path], output_dir: Path) -> list[str]:
    if not winmd_inputs:
        fail("windows-bindgen native emit requires at least one raw .winmd input")
...
def emit_transient_winmd_json(winmd_inputs: Sequence[Path], output_dir: Path, timeout_seconds: int) -> None:
    ...
    require_success(result, "windows-bindgen native winmd metadata read")
...
def self_test_source_hash_check() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="windows-common-source-hash-test-") as temp:
        fake_root = Path(temp)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if needs_type_metadata(args):
            ...
            with tempfile.TemporaryDirectory(prefix="windows-common-winmd-meta-") as temp:
