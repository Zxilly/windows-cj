...
GENERATED_BY = windows_common_manifest.GENERATED_MANIFEST_HEADER
IGNORED_GENERATED_FILES = windows_common_manifest.IGNORED_GENERATED_FILES
...
def load_manifest(root: Path = WINDOWS_COMMON) -> dict:
    return windows_common_manifest.load_manifest(root, expected_header=GENERATED_BY)


def manifest_files(manifest: dict) -> list[str]:
    return windows_common_manifest.manifest_files(manifest)


def manifest_hashes(manifest: dict) -> dict[str, str]:
    return windows_common_manifest.manifest_hashes(manifest)


def generated_tree_files(root: Path) -> set[str]:
    return windows_common_manifest.generated_tree_files(root)


def check_manifest_hashes(root: Path = WINDOWS_COMMON) -> dict:
    manifest = windows_common_manifest.validate_manifest_hashes(root, expected_header=GENERATED_BY)
...
def manifest_requested_features(manifest: dict) -> list[str]:
    try:
        return windows_common_manifest.requested_features(manifest, require_nonempty=True)
...
def impl_symbol_scan(root: Path) -> ImplSymbolScan:
    ...
    for path in sorted((root / "src" / "impl").glob("symbols_*.cj")):
        ...
        relative = windows_common_manifest.rel_posix(path, root)
...
def self_test_normalization() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="windows_common-manifest-test-") as temp:
        root = Path(temp)
        (root / "src").mkdir()
        ...
        assert windows_common_manifest.validate_manifest_hashes(root)["files"] == ["src/a.cj"]
